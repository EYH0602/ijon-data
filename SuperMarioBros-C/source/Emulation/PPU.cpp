...
PPU::PPU(SMBEngine &engine) : engine(engine) {
  currentAddress = 0;
  writeToggle = false;
}
...
uint8_t PPU::getAttributeTableValue(uint16_t nametableAddress) {
  ...
  int shift = ((nametableAddress & (1 << 6)) ? 4 : 0) +
              ((nametableAddress & (1 << 1)) ? 2 : 0);
      ...
      for (int row = 0; row < 8; row++) {
        ...
        for (int column = 0; column < 8; column++) {
          uint8_t paletteIndex = (((plane1 & (1 << column)) ? 1 : 0) +
                                  ((plane2 & (1 << column)) ? 2 : 0));
          uint8_t colorIndex =
              palette[0x10 + (attributes & 0x03) * 4 + paletteIndex];
          if (paletteIndex == 0) {
            // Skip transparent pixels
            continue;
          }
...
void PPU::writeAddressRegister(uint8_t value) {
  if (!writeToggle) {
    // Upper byte
    currentAddress =
        (currentAddress & 0xff) | (((uint16_t)value << 8) & 0xff00);
  } else {
    // Lower byte

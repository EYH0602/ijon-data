...
public:
   ...
   *
   * @param mode the mode to run. 0 runs initialization routines, 1 runs the
   * logic for frames.
   */
  void code(int mode);
  ...
  /**
   * Get a word of memory from a zero-page address and the next byte (wrapped
   * around), in little-endian format.
   */
  ...
  /**
   * Map constant data to the address space. The address must be at least
   * 0x8000.
   */

...
SkipSprite0: // set scroll registers from variables
  ...
  writeData(PPU_CTRL_REG1, a);
  a = M(
      GamePauseStatus); // if in pause mode, do not perform operation mode stuff
  a >>= 1;
  if (c)
    goto SkipMainOper;
  JSR(OperModeExecutionTree,
      12); // otherwise do one of many, many possible subroutines
...
ShuffleLoop: // check for offset value against
  ...
  y = M(
      SprShuffleAmtOffset); // get current offset to preset value we want to add
  c = 0;
...
SetMiscOffset: // load one of three OAM data offsets
  ...
  writeData(Misc_SprDataOffset - 1 + x,
            a); // note that due to the way X is set up,
...
ChkSelect: // check to see if the select button was pressed
  ...
  if (!z)
    goto ChkWorldSel;        // if demo timer not expired, branch to check world
                             // selection
...
SelectBLogic: // if select or B pressed, check demo timer one last time
  ...
  if (z)
    goto IncWorldSel;     // note this will not be run if world selection is
                          // disabled
  a = M(NumberOfPlayers); // if no, must have been the select button, therefore
  a ^= BOOST_BINARY(
      00000001); // change number of players and draw icon accordingly
...
RunDemo: // run game engine
  ...
  a = M(
      GameEngineSubroutine); // check to see if we're running lose life routine
  compare(a, 0x06);
  if (!z)
...
ThankPlayer: // put primary message counter into Y
  y = a;
  if (!z)
    goto SecondPartMsg; // if counter nonzero, skip this part, do not print
                        // first message
...
EvalForMusic: // if counter not yet at 3 (world 8 only), branch
  ...
  writeData(EventMusicQueue,
            a); // otherwise load victory music first (world 8 only)
...
PrintMsg: // put primary message counter in A
  ...
  writeData(VRAM_Buffer_AddrCtrl,
            a); // write message counter to vram address controller
...
ChkNumTimer: // use as Y
  ...
  writeData(FloateyNum_Control + x,
            a); // initialize floatey number control and leave
  goto Return;
...
InitScreen:
  JSR(MoveAllSpritesOffscreen,
      29);                       // initialize all sprites including sprite #0
...
GetBackgroundColor:
  ...
  writeData(VRAM_Buffer_AddrCtrl,
            a); // note that if set to 5-7, $0301 will not be read
...
ClrGetLoop: // fetch player colors and store them
  ...
  y = M(
      BackgroundColorCtrl); // if this value is four or greater, it will be set
  if (!z)
...
DisplayIntermediate:
  ...
  if (z)
    goto PlayerInter;
  a = M(DisableIntermediate); // if this flag is set, skip intermediate lives
                              // display
  if (!z)
...
CheckPlayerName:
  ...
  if (z)
    goto ChkLuigi;
  a ^= BOOST_BINARY(
      00000001); // if not, must be time up, invert d0 to do other player
...
RenderAreaGraphics:
  ...
  writeData(0x00, y);
  a = M(CurrentNTAddr_Low); // get current name table address we're supposed to
                            // render
...
DrawMTLoop: // store init value of 0 or incremented offset for buffer
  writeData(0x01, x);
  a = M(MetatileBuffer +
        x); // get first metatile number, and mask out all but 2 MSB
  ...
  a &= BOOST_BINARY(
      00000001); // mask out all but LSB, then invert LSB, multiply by 2
  a ^= BOOST_BINARY(
      00000001); // to get the correct column position in the metatile,
  ...
  a = M(W(0x06) + y);
  writeData(VRAM_Buffer2 + 3 + x,
            a); // get first tile number (top left or top right) and store
  ++y;
  a = M(W(0x06) +
        y); // now get the second (bottom left or bottom right) and store
...
SetAttrib: // get previously saved bits from before
  ...
  ++y;
  a = 0x00;
  writeData(VRAM_Buffer2 + y,
            a); // put null terminator at end of data for name table
  ...
  a = M(
      CurrentNTAddr_High); // and then invert d2 of the name table address high
...
AttribLoop:
  ...
  writeData(VRAM_Buffer2_Offset,
            y); // store offset in case we want to do any more
...
ColorRotation:
  ...
  if (!z)
    goto ExitColorRot; // branch if not set to zero to do this every eighth
                       // frame
...
GetAreaPal: // fetch palette to be written based on area type
  a = M(Palette3Data + y);
  writeData(VRAM_Buffer1 + 3 + x,
            a); // store it to overwrite blank palette in vram buffer
  ++y;
  ++x;
  ...
  writeData(VRAM_Buffer1 + 4 + x,
            a); // get and store current color in second slot of palette
...
WriteBlankMT: // do a sub to write blank metatile to vram buffer
  ...
  a = 0x06;
  writeData(VRAM_Buffer_AddrCtrl,
            a); // set vram address controller to $0341 and leave
  goto Return;
...
ReplaceBlockMetatile:
  JSR(WriteBlockMetatile,
      43); // write metatile to vram buffer to replace block object
...
UseBOffset: // put Y in A
  ...
  JSR(PutBlockMetatile,
      44); // get appropriate block data and write to vram buffer
...
InitializeNameTables:
  ...
  a |= BOOST_BINARY(
      00010000); // set sprites for first 4k and background for second 4k
...
Save8Bits:
  pla();
  writeData(JoypadBitMask + x,
            a); // save with all bits in another place and leave
  goto Return;
...
UpdateScreen: // reset flip-flop
  ...
  a = M(W(0x00) + y);
  if (!z)
    goto WriteBufferToScreen; // if byte is zero we have no further updates to
                              // make here
...
CarryOne: // subtract ten from our digit to make it a
  ...
  ++M(DigitModifier - 1 +
      x);         // preceding current digit to "carry the one" properly
...
GetScoreDiff: // subtract each player digit from each high score digit
  ...
  a -= M(TopScoreDisplay +
         y); // from lowest to highest, if any top score digit exceeds
  --x;       // any player digit, borrow will be set until a subsequent
  --y;       // subtraction clears it (player digit is higher than top)
  if (!n)
    goto GetScoreDiff;
  if (!c)
    goto NoTopSc; // check to see if borrow is still set, if so, no new high
                  // score
...
ClrTimersLoop: // clear out memory between
  ...
  if (!n)
    goto ClrTimersLoop;
  a = M(HalfwayPage);
  y = M(AltEntranceControl); // if AltEntranceControl not set, use halfway page,
                             // if any found
  if (z)
    goto StartPage;
...
StartPage: // set as value here
  ...
  writeData(
      BackloadingFlag,
      a); // set flag here if halfway page or saved entry page number found
  ...
  a &= BOOST_BINARY(
      00000001); // otherwise use $2080, this address used later as name table
  if (z)
...
SetInitNTHigh: // store name table address
  ...
  writeData(ColumnSets,
            a); // 12 column sets = 24 metatile columns = 1 1/2 screens
  JSR(GetAreaDataAddrs, 55); // get enemy and level addresses and load header
  a = M(
      PrimaryHardMode); // check to see if primary hard mode has been activated
  if (!z)
...
PrimaryGameSetup:
  a = 0x01;
  writeData(FetchNewGameTimerFlag,
            a);             // set flag to load game timer from header
...
ClearVRLoop: // clear buffer at $0300-$03ff
  ...
  a = 0xff;
  writeData(BalPlatformAlignment,
            a);                  // initialize balance platform assignment flag
...
Entrance_GameTimerSetup:
  ...
  writeData(VerticalForceDown,
            a); // for fractional movement downwards if necessary
  a = 0x01;     // set high byte of player position and
  writeData(PlayerFacingDir,
            a); // set facing direction so that player faces right
...
ChkStPos:
  ...
  if (z)
    goto SetStPos;
  x = M(AltYPosOffset - 2 +
        y); // if not 0 or 1, override $0710 with new offset in X

SetStPos: // load appropriate horizontal position
  a = M(PlayerStarting_X_Pos + y);
  writeData(Player_X_Position,
            a); // and vertical positions for the player, using
  a = M(PlayerStarting_Y_Pos +
        x); // AltEntranceControl as offset for horizontal and either $0710
  writeData(Player_Y_Position,
            a); // or value that overwrote $0710 as offset for vertical
  a = M(PlayerBGPriorityData + x);
  writeData(Player_SprAttrib,
            a);             // set player sprite attributes using offset in X
  ...
  a = M(GameTimerData +
        y); // if game timer is set and game timer flag is also set,
  writeData(GameTimerDisplay,
            a); // use value of game timer control for first digit of game timer
  a = 0x01;
...
IncrementColumnPos:
  ...
  if (!z)
    goto NoColWrap;
  writeData(CurrentColumnPos,
            a);        // if no bits left set, wrap back to zero (0-f)
...
AreaParserCore:
  ...
  if (z)
    goto RenderSceneryTerrain; // if not, go ahead and render background,
                               // foreground and terrain
...
SceLoop1: // load metatile data from offset of (lsb - 1) * 3
  ...
  writeData(MetatileBuffer + y,
            a); // store into buffer from offset of (msb / 16)
  ++x;
  ++y;
...
RendFore: // check for foreground data needed or not
  ...
  y = M(FSceneDataOffsets - 1 +
        x); // load offset from location offset by header value, then
  x = 0x00; // reinit X
...
TerrBChk: // load bitmask, then perform AND on contents of first byte
  ...
  bit(M(0x00));
  if (z)
    goto NextTBit; // if not set, skip this part (do not write terrain to
                   // buffer)
  a = M(0x07);
  writeData(MetatileBuffer + x,
            a); // load terrain type metatile number and store into buffer here
...
ChkMTLow:
  ...
  compare(a, M(BlockBuffLowBounds +
               y)); // check for certain values depending on bits set
  if (c)
...
Chk1Row13:
  ...
  a &= BOOST_BINARY(
      01000000); // check for d6 set (if not, object is page control)
  if (!z)
    goto CheckRear;
  ...
  a = M(W(AreaData) + y);
  a &= BOOST_BINARY(
      00011111); // mask out all but 5 LSB and store in page control
...
ProcLoopb: // decrement buffer offset
  ...
  a = M(BehindAreaParserFlag); // check for flag set if objects were behind
                               // renderer
  if (!z)
...
NormObj: // store value here (branch for small objects and rows 13 and 14)
  ...
  a = M(AreaObjectPageLoc); // otherwise check to see if the object we've loaded
                            // is on the
...
AlterAreaAttributes:
  y = M(AreaObjOffsetBuffer +
        x); // load offset for level object data saved in buffer
  ++y;      // load second byte
  a = M(W(AreaData) + y);
...
WarpNum:
  a = x;
  writeData(WarpZoneControl,
            a);           // store number here to be used by warp zone routine
...
AreaFrenzy: // use area object identifier bit as offset
  x = M(0x00);
  a = M(FrenzyIDData - 8 +
        x); // note that it starts at 8, thus weird address here
  y = 0x05;

FreCompLoop: // check regular slots of enemy object buffer
  --y;
  if (n)
    goto ExitAFrenzy; // if all slots checked and enemy object not found, branch
                      // to store
  compare(a, M(Enemy_ID +
               y)); // check for enemy object in buffer versus frenzy object
  if (!z)
    goto FreCompLoop;
...
TreeLedge:
  ...
  if (!n)
    goto MidTreeL;
  a = y;
  writeData(AreaObjectLength + x,
            a); // store lower nybble into buffer flag as length of ledge
  a = M(CurrentPageLoc);
...
MidTreeL:
  ...
  writeData(MetatileBuffer + x,
            a);  // note that this is also used if ledge position is
...
EndMushL: // if at the end, render end of mushroom
  ...
  a = 0x4f;
  writeData(MetatileBuffer + x,
            a); // render stem top of mushroom underneath the middle
  a = 0x50;
...
NotTall: // if not tall castle, check to see if we're at the third column
  compare(a, 0x02);
  if (!z)
    goto ExitCastle; // if we aren't and the castle is tall, don't create flag
                     // yet
  JSR(GetAreaObjXPosition,
      81); // otherwise, obtain and save horizontal pixel coordinate
  pha();
  JSR(FindEmptyEnemySlot, 82); // find an empty place on the enemy object buffer
  pla();
  writeData(Enemy_X_Position + x,
            a); // then write horizontal coordinate for star flag
  a = M(CurrentPageLoc);
...
PlayerStop: // put brick at floor to stop player at end of level
  y = 0x52;
  writeData(MetatileBuffer + 10,
            y); // this is only done if we're on the second column
...
WaterPipe:
  ...
  y = M(AreaObjectLength +
        x);    // get length (residual code, water pipe is 1 col thick)
  x = M(0x07); // get row
  a = 0x6b;
...
VPipeSectLoop: // all the way to the top of the screen
  a = 0x00;
  writeData(MetatileBuffer + x,
            a); // because otherwise it will look like exit pipe
  --x;
  if (!n)
    goto VPipeSectLoop;
...
ExitPipe:
  ...
  JSR(GetLrgObjAttrib,
      87); // get vertical length, then plow on through RenderSidewaysPipe
...
RenderSidewaysPipe:
  ...
  ++x;
  a = M(SidePipeShaftData +
        y); // check for value $00 based on horizontal offset
  compare(a, 0x00);
  if (z)
...
WarpPipe: // save value in stack
  ...
  JSR(GetAreaObjYPosition,
      92); // get piranha plant's vertical coordinate and store here
...
DrawPipe: // get value saved earlier and use as Y
  ...
  a = M(VerticalPipeData +
        y); // draw the appropriate pipe with the Y we loaded earlier
...
FlagpoleObject:
  ...
  a = 0xb0;
  writeData(FlagpoleFNum_Y_Pos,
            a); // set initial vertical coordinate for flagpole's floatey number
  a = FlagpoleFlagObject;
  writeData(Enemy_ID + 5,
            a); // set flag identifier, note that identifier and coordinates
...
Hole_Empty:
  ...
  JSR(GetAreaObjXPosition,
      121); // get proper vertical coordinate of where we're at
  c = 1;
  ...
  if (!c)
    goto StrWOffset; // if not yet reached fifth whirlpool, branch to save
                     // offset
  x = 0x00;          // otherwise initialize it
...
RenderUnderPart:
  ...
  compare(y, 0xc0);
  if (c)
    goto WaitOneRow; // if any other metatile with palette 3, wait until next
                     // row
  compare(y, 0x54);
  if (!z)
...
WaitOneRow:
  ...
  if (c)
    goto ExitUPartR;
  y = M(AreaObjectHeight); // decrement, and stop rendering if there is no more
                           // length
  --y;
  if (!n)
    goto RenderUnderPart;
...
ChkLrgObjLength:
  JSR(GetLrgObjAttrib,
      122); // get row location and size (length if branched to from here)
...
GetLrgObjAttrib:
  y = M(AreaObjOffsetBuffer +
        x);               // get offset saved from area obj decoding routine
...
GetAreaDataAddrs:
  ...
  a = M(AreaDataHOffsets +
        y); // do the same thing but with different base value
  c = 0;
  ...
  a &= BOOST_BINARY(
      00000111); // save 3 LSB for foreground scenery or bg color control
  compare(a, 0x04);
  if (!c)
    goto StoreFore;
  writeData(BackgroundColorCtrl,
            a); // if 4 or greater, save value here as bg color control
  a = 0x00;
...
StoreFore: // if less, save value here as foreground scenery
  ...
  a >>= 1;
  a >>= 1;
  writeData(PlayerEntranceCtrl,
            a);                // save value here as player entrance control
...
ProcELoop: // put incremented offset in X as enemy object offset
  ...
  JSR(RelativePlayerPosition,
      130);                   // get relative coordinates for player object
  JSR(PlayerGfxHandler, 131); // draw the player
  JSR(BlockObjMT_Updater,
      132); // replace block objects with metatiles if necessary
  x = 0x01;
  ...
  compare(a,
          0x02); // if player is below the screen, don't bother with the music
  if (!n)
    goto NoChgMus;
...
CycleTwo: // if branched here, divide by 2 to cycle every other frame
  ...
  JSR(CyclePlayerPalette,
      142);    // do sub to cycle the palette (note: shares fire flower code)
...
SaveAB: // save current A and B button
  a = M(A_B_Buttons);
  writeData(PreviousA_B_Buttons,
            a); // into temp variable to be used on next frame
  a = 0x00;
  writeData(Left_Right_Buttons,
            a); // nullify left and right buttons temp variable
...
ScrollHandler:
  ...
  writeData(Player_X_Scroll,
            a);      // save as new value here to impose force on scroll
...
ScrollScreen:
  ...
  writeData(Mirror_PPU_CTRL_REG1,
            a);                // mirror to be used to set name table later
  JSR(GetScreenPosition, 145); // figure out where the right side is
  a = 0x08;
  writeData(ScrollIntervalTimer,
            a);    // set scroll timer (residual, not used elsewhere)
...
KeepOnscr: // get left or right side coordinate based on offset
  ...
  writeData(Player_X_Position,
            a); // store as player position to prevent movement further
  a = M(ScreenEdge_PageLoc +
        y);  // get left or right page location based on offset
  a -= 0x00; // subtract borrow
...
GetScreenPosition:
  ...
  writeData(ScreenRight_X_Pos,
            a);              // store as coordinate of screen's right boundary
  a = M(ScreenLeft_PageLoc); // get page number where left boundary is
  a += 0x00;                 // add carry from before
  writeData(ScreenRight_PageLoc,
            a); // store as page number where right boundary is
  goto Return;

  //------------------------------------------------------------------------

GameRoutines:
  a = M(GameEngineSubroutine); // run routine based on number (a few of these
                               // routines are
  switch (a) {
  case 0:
...
EntrMode2: // if controller override bits set here,
  ...
  a = M(
      Player_Y_Position); // check to see if player is at a specific coordinate
...
VineEntr:
  ...
  writeData(Block_Buffer_1 + 0xb4,
            a); // use same value to force player to climb

OffVine: // set collision detection disable flag
  writeData(DisableCollisionDet, y);
  JSR(AutoControlPlayer,
      149); // use contents of A to move player up or right, execute sub
...
AutoControlPlayer:
  writeData(SavedJoypadBits,
            a); // override controller bits with contents of A if executing here

PlayerCtrlRoutine:
  a = M(GameEngineSubroutine); // check task here
  compare(
      a,
      0x0b); // if certain value is set, branch to skip controller bit loading
  if (z)
    goto SizeChk;
...
VerticalPipeEntry:
  ...
  JSR(ScrollHandler,
      158); // do sub to scroll screen with saved force if necessary
...
ChgAreaPipe: // decrement timer for change of area
  ...
  if (!z)
    goto ExitCAPipe;
  writeData(AltEntranceControl,
            y); // when timer expires set mode of alternate entry
...
RightPipe: // use contents of Y to
  a = y;
  JSR(AutoControlPlayer,
      160); // execute player control routine with ctrl bits nulled
  goto Return;
...
EndChgSize: // check again for another specific moment
  ...
  JSR(DonePlayerTask,
      161); // otherwise do sub to init timer control and set routine
...
InitChangeSize:
  ...
  writeData(PlayerAnimCtrl,
            y); // otherwise initialize player's animation frame control
...
DonePlayerTask:
  a = 0x00;
  writeData(TimerControl,
            a); // initialize master timer control to continue timers
  a = 0x08;
  writeData(GameEngineSubroutine,
            a); // set player control routine to run next frame
  goto Return;  // leave
...
ResetPalFireFlower:
  JSR(DonePlayerTask,
      162); // do sub to init timer control and run player control routine
...
FlagpoleSlide:
  ...
  if (c)
    goto SlidePlayer; // far enough, and if so, branch with no controller bits
                      // set
...
ChkStop: // get player collision bits
  ...
  ++M(StarFlagTaskControl); // otherwise set task control now (this gets ball
                            // rolling!)
...
RdyNextA:
  ...
  compare(a, M(Hidden1UpCoinAmts +
               y)); // against minimum value, if player has not collected
  if (!c)
...
NextArea: // increment area number used for address loader
  ...
  JSR(ChgAreaMode,
      165); // do sub to set secondary mode, disable screen and sprite 0
...
GndMove: // do a sub to impose friction on player's walk/run
  ...
  JSR(MovePlayerHorizontally,
      169); // do another sub to move player horizontally
  writeData(Player_X_Scroll,
            a); // set returned value as player's movement speed for scroll
  goto Return;
...
FallingSub:
  ...
  writeData(VerticalForce,
            a); // dump vertical movement force for falling into main one
...
JumpSwimSub:
  ...
  c = 1;
  a -= M(
      Player_Y_Position); // subtract current from original vertical coordinate
  compare(a, M(DiffToHaltJump)); // compare to value set here to see if player
                                 // is in mid-jump
  if (!c)
...
JSMove: // do a sub to move player horizontally
  ...
  writeData(Player_X_Scroll,
            a); // set player's speed here, to be used for scroll later
  a = M(GameEngineSubroutine);
...
CSetFDir:
  ...
  writeData(PlayerFacingDir,
            a); // is on vine to face player in opposite direction
...
PlayerPhysicsSub:
  ...
  a &= M(
      Player_CollisionBits); // check against player's collision detection bits
  if (z)
...
CheckForJumping:
  ...
  a &=
      M(PreviousA_B_Buttons); // if button not pressed in previous frame, branch
  if (z)
    goto ProcJumping;
...
GetXPhy2: // get maximum speed to the right
  ...
  compare(
      a,
      M(Player_MovingDir)); // check facing direction against moving direction
  if (z)
    goto ExitPhy; // if the same, branch to leave
  M(FrictionAdderLow) <<=
      1; // otherwise shift d7 of friction adder low into carry
  M(FrictionAdderHigh)
      .rol(); // then rotate carry onto d0 of friction adder high
...
GetPlayerAnimSpeed:
  ...
  if (c)
    goto ChkSkid; // if greater than this but not greater than first, skip
                  // increment
...
ProcSkid: // check player's walking/running speed
  ...
  a = M(PlayerFacingDir);
  writeData(Player_MovingDir,
            a); // otherwise use facing direction to set moving direction
  a = 0x00;
...
ImposeFriction:
  a &= M(Player_CollisionBits); // perform AND between left/right controller
                                // bits and collision flag
  compare(a, 0x00); // then compare to zero (this instruction is redundant)
  if (!z)
    goto JoypFrict; // if any bits set, branch to next part
  a = M(Player_X_Speed);
  if (z)
    goto SetAbsSpd; // if player has no horizontal speed, branch ahead to last
                    // part
  if (!n)
    goto RghtFrict; // if player moving to the right, branch to slow
  if (n)
    goto LeftFrict; // otherwise logic dictates player moving left, branch to
                    // slow
...
LeftFrict: // load value set here
  ...
  compare(
      a,
      M(MaximumRightSpeed)); // compare against maximum value for right movement
  if (n)
...
RghtFrict: // load value set here
  ...
  compare(
      a,
      M(MaximumLeftSpeed)); // compare against maximum value for left movement
  if (!n)
...
ProcFireball_Bubble:
  ...
  --y;
  writeData(PlayerAnimTimer,
            y);         // decrement and store in player's animation timer
...
ProcFireballs:
  ...
  x = 0x01;
  JSR(FireballObjCore,
      174); // process second fireball object, then do air bubbles
...
FireballObjCore:
  ...
  a <<= 1;
  if (c)
    goto FireballExplosion;  // if so, branch to get relative coordinates and
                             // draw explosion
  ...
  a = 0x07;
  writeData(Fireball_BoundBoxCtrl + x,
            a);            // set bounding box size control for fireball
...
RunFB: // add 7 to offset to use
  ...
  a = 0x00;
  JSR(ImposeGravity,
      179); // do sub here to impose gravity on fireball and move vertically
  ...
  JSR(FireballBGCollision,
      184);                    // do fireball to background collision detection
  ...
  JSR(FireballEnemyCollision,
      185); // do fireball to enemy collision detection and deal with collisions
...
PosBubl: // use value loaded as adder
  ...
  writeData(Bubble_X_Position + x,
            a); // save as horizontal position for airbubble
  a = M(Player_PageLoc);
  ...
  a += 0x08;
  writeData(Bubble_Y_Position + x,
            a); // save as vertical position for air bubble
  a = 0x01;
...
TimeUpOn: // init player status (note A will always be zero here)
  writeData(PlayerStatus, a);
  JSR(ForceInjury,
      188); // do sub to kill the player (note player is small here)
...
WarpZoneObject:
  ...
  ++M(WarpZoneControl);  // increment warp zone flag to make warp pipes for warp
                         // zone
...
WhLoop: // get left extent of whirlpool
  ...
  if (!n)
    goto WhirlpoolActivate; // if player within right extent, branch to
                            // whirlpool code
...
WhirlpoolActivate:
  ...
  a = M(Player_X_Position); // otherwise slowly pull player left, towards the
                            // center
  c = 1;
...
LeftWh: // get player's collision bits
  ...
  a = M(Player_X_Position); // otherwise slowly pull player right, towards the
                            // center
  c = 0;
...
WhPull:
  ...
  goto ImposeGravity; // jump to put whirlpool effect on player vertically, do
                      // not return
...
GiveFPScr: // get score offset from earlier (when player touched flagpole)
  ...
  JSR(AddToScore,
      189); // do sub to award player points depending on height of collision
  a = 0x05;
  writeData(GameEngineSubroutine,
            a); // set to run end-of-level subroutine on next frame
...
BounceJS: // check frame control offset again
  ...
  a = M(JumpspringForce);
  writeData(Player_Y_Speed,
            a); // store jumpspring force as player's new vertical speed
  a = 0x00;
...
Setup_Vine:
  ...
  writeData(Enemy_X_Position + x,
            a); // copy horizontal coordinate from previous object
  a = M(Block_Y_Position + y);
  writeData(Enemy_Y_Position + x,
            a);          // copy vertical coordinate from previous object
  ...
  writeData(VineStart_Y_Position,
            a); // otherwise store vertical coordinate here
...
WrCMTile: // check vine height
  ...
  JSR(BlockBufferCollision,
      201); // do a sub to get block buffer address set, return contents
  y = M(0x02);
  ...
  a = 0x26;
  writeData(W(0x06) + y,
            a); // otherwise, write climbing metatile to block buffer
...
FireCannon:
  ...
  a = 0x09;
  writeData(Enemy_BoundBoxCtrl + x,
            a); // set bounding box size control for bullet bill
  a = BulletBill_CannonVar;
  writeData(Enemy_ID + x,
            a);  // load identifier for bullet bill (cannon variant)
...
Chk_BB: // check enemy identifier for bullet bill (cannon variant)
  ...
  JSR(OffscreenBoundsCheck,
      202);              // otherwise, check to see if it went offscreen
  a = M(Enemy_Flag + x); // check enemy buffer flag
  if (z)
...
BulletBillHandler:
  ...
  JSR(PlayerEnemyDiff,
      205); // get horizontal difference between player and bullet bill
  if (n)
...
SetupBB: // set bullet bill's moving direction
  ...
  a = M(BulletBillXSpdData +
        y); // get horizontal speed based on moving direction
...
ChkDSte: // check enemy state for d5 set
  ...
  JSR(MoveD_EnemyVertically,
      206); // otherwise do sub to move bullet bill vertically
...
SetMOfs: // use either d3 or d2-d0 for offset here
  ...
  x = M(HammerEnemyOfsData +
        y);              // get offset of enemy slot to check using Y as offset
...
ProcHammerObj:
  ...
  a &= BOOST_BINARY(01111111); // mask out d7
  y = M(HammerEnemyOffset +
        x);         // get enemy object offset that spawned this hammer
  ...
  JSR(ImposeGravity,
      213); // do sub to impose gravity on hammer and move vertically
...
SetHSpd:
  ...
  a = M(HammerXSpdData +
        x);            // get proper speed to use based on moving direction
...
CoinBlock:
  JSR(FindEmptyMiscSlot,
      220); // set offset for empty or last misc object buffer slot
  ...
  a |= 0x05;                      // add 5 pixels
  writeData(Misc_X_Position + y,
            a);                // store as horizontal coordinate of misc object
  a = M(Block_Y_Position + x); // get vertical coordinate of block object
  a -= 0x10;                   // subtract 16 pixels
  writeData(Misc_Y_Position + y,
            a); // store as vertical coordinate of misc object
  goto JCoinC;  // jump to rest of code as applies to this misc object

SetupJumpCoin:
  JSR(FindEmptyMiscSlot,
      221); // set offset for empty or last misc object buffer slot
  ...
  writeData(Misc_X_Position + y,
            a); // save as horizontal coordinate for misc object
...
JCoinC:
  ...
  JSR(GiveOneCoin,
      222); // update coin tally on the screen and coin amount variable
  ++M(CoinTallyFor1Ups); // increment coin tally used to activate 1-up block
                         // flag
  goto Return;
...
MiscLoop: // store misc object offset here
  ...
  if (!c)
    goto ProcJumpCoin; // if d7 not set, jumping coin, thus skip to rest of code
                       // here
...
ProcJumpCoin:
  ...
  ++M(Misc_State +
      x); // otherwise increment state to either start off or as timer
...
JCoinRun:
  ...
  JSR(ImposeGravity,
      224); // do sub to move coin vertically and impose gravity on it
  x = M(ObjectOffset);     // get original misc object offset
...
AddToScore:
  ...
  JSR(DigitsMathRoutine,
      230); // update the score internally with value in digit modifier

GetSBNybbles:
  y = M(CurrentPlayer); // get current player
  a = M(StatusBarNybbles +
        y); // get nybbles based on player, use to update score and coins

UpdateNumber:
  JSR(PrintStatusBarNumbers,
      231); // print status bar numbers based on nybbles, whatever they be
...
SetupPowerUp:
  ...
  writeData(Enemy_X_Position + 5,
            a); // as horizontal coordinate of power-up object
  a = 0x01;
  writeData(Enemy_Y_HighPos + 5,
            a);                // set vertical high byte of power-up object
  a = M(Block_Y_Position + x); // get vertical coordinate of block object
  c = 1;
  a -= 0x08; // subtract 8 pixels
  writeData(Enemy_Y_Position + 5,
            a); // and use as vertical coordinate of power-up object
...
PwrUpJmp: // this is a residual jump point in enemy object jump table
  ...
  a = 0x03;
  writeData(Enemy_BoundBoxCtrl + 5,
            a); // set bounding box size control for power-up object
  a = M(PowerUpType);
...
PowerUpObjHandler:
  ...
  JSR(MoveJumpingEnemy,
      232); // otherwise impose gravity on star power-up and make it jump
...
GrowThePowerUp:
  ...
  ++M(Enemy_State +
      5);           // increment state for next frame (to make power-up rise)
  ...
  writeData(Enemy_SprAttrib + 5,
            a); // initialize background priority bit set here
...
DBlockSte: // store into block object buffer
  ...
  JSR(DestroyBlockMetatile,
      242); // store blank metatile in vram buffer to write to name table
  x = M(SprDataOffset_Ctrl); // load offset control bit
  a = M(0x02); // get vertical high nybble offset used in block buffer routine
  writeData(Block_Orig_YPos + x,
            a); // set as vertical coordinate for block object
  y = a;
  ...
  JSR(BlockBumpedChk,
      243);           // do a sub to check which block player bumped head on
...
BigBP: // get player's vertical coordinate
  ...
  writeData(Block_Y_Position + x,
            a);           // save as vertical coordinate for block object
...
InitBlock_XY_Pos:
  ...
  writeData(Block_X_Position + x,
            a); // save as horizontal coordinate for block object
  a = M(Player_PageLoc);
...
BumpBlock:
  JSR(CheckTopOfBlock,
      247); // check to see if there's a coin directly above this block
  a = Sfx_Bump;
  writeData(Square1SoundQueue, a); // play bump sound
  a = 0x00;
  writeData(Block_X_Speed + x,
            a); // initialize horizontal speed for block object
  ...
  JSR(BlockBumpedChk,
      248); // do a sub to check which block player bumped head on
  if (!c)
...
BrickShatter:
  JSR(CheckTopOfBlock,
      250); // check to see if there's a coin directly above this block
  a = Sfx_BrickShatter;
  writeData(Block_RepFlag + x,
            a); // set flag for block object to immediately replace metatile
  ...
  a = 0x05;
  writeData(DigitModifier + 5,
            a);              // set digit modifier to give player 50 points
...
CheckTopOfBlock:
  ...
  JSR(SetupJumpCoin,
      254); // create jumping coin object and update coin variables
...
SpawnBrickChunks:
  ...
  a = 0xf0;
  writeData(Block_X_Speed + x,
            a); // set horizontal speed for brick chunk objects
  ...
  a = 0x00;
  writeData(Block_Y_MoveForce + x,
            a); // init fractional movement force for both
...
BlockObjectsCore:
  ...
  if (z)
    goto BouncingBlockHandler; // branch if found, otherwise continue for brick
                               // chunks
  JSR(ImposeGravityBlock,
      255); // do sub to impose gravity on one block object object
  ...
  x = a;
  JSR(ImposeGravityBlock,
      257); // do sub to impose gravity on other block object
  ...
  a = 0xf0;
  compare(a, M(Block_Y_Position + 2 +
               x)); // check to see if bottom block object went
  if (c)
...
BouncingBlockHandler:
  ...
  if (c)
    goto UpdSte; // if still above amount, not time to kill block yet, thus
                 // branch
  a = 0x01;
...
UpdateLoop: // set offset here
  ...
  a = M(Block_Orig_YPos +
        x);           // get original vertical coordinate of block object
  ...
  JSR(ReplaceBlockMetatile,
      266); // do sub to replace metatile where block object is
  a = 0x00;
...
UseAdder: // save Y here
  ...
  writeData(SprObject_X_Position + x,
            a); // plus $f0 if necessary) to object's horizontal position
...
MovePlayerVertically:
  ...
  a = M(
      JumpspringAnimCtrl); // otherwise check to see if jumpspring is animating
  if (!z)
...
MoveD_EnemyVertically:
  ...
  if (!z)
    goto ContVMove; // and use, otherwise set different movement amount,
                    // continue on
...
AlterYP: // store Y here
  writeData(0x07, y);
  a += M(SprObject_Y_Position +
         x); // add vertical position to vertical speed plus carry
  ...
  if (!c)
    goto ChkUpM;
  a = M(0x02);
  writeData(SprObject_Y_Speed + x,
            a); // keep vertical speed within maximum value
  a = 0x00;
...
ChkUpM: // get value from stack
  ...
  a = M(0x02);
  a ^=
      BOOST_BINARY(11111111); // otherwise get two's compliment of maximum speed
  y = a;
  ++y;
  ...
  writeData(SprObject_Y_MoveForce + x,
            a); // thus it effectively undoes add we did earlier
...
ChkBowserF: // get data from stack
  ...
  y = a;
  a = M(Enemy_Flag +
        y); // use as pointer and load same place with different offset
  if (!z)
    goto ExitELCore;
  writeData(Enemy_Flag + x,
            a); // if second enemy flag not set, also clear first one
...
FindLoop:
  ...
  if (!z)
    goto FindLoop;
  a = M(Player_Y_Position); // check to see if the player is at the correct
                            // position
...
CheckRightBounds:
  ...
  ++y;
  a = M(W(EnemyData) +
        y); // if MSB of enemy object is clear, branch to check for row $0f
  a <<= 1;
  if (!c)
...
CheckPageCtrlRow:
  ...
  a &= BOOST_BINARY(00111111);
  writeData(EnemyObjectPageLoc,
            a);         // store as page control for enemy object data
...
PositionEnemyObj:
  ...
  compare(a,
          M(ScreenRight_X_Pos)); // check column position against right boundary
...
ParseRow0e:
  ...
  a &= BOOST_BINARY(
      00011111); // the 3 MSB from before, save as page number to be
...
CheckpointEnemyID:
  ...
  writeData(Enemy_Y_Position + x,
            a); // enemy object's vertical coordinate ($00-$14 only)
  a = 0x01;
...
InitHammerBro:
  ...
  writeData(EnemyIntervalTimer + x,
            a); // set value as delay for hammer bro to walk left
...
InitRedPTroopa:
  ...
  writeData(RedPTroopaOrigXPos + x,
            a); // be used as original vertical coordinate
  if (!n)
...
GetCent: // send central position adder to A
  ...
  writeData(RedPTroopaCenterYPos + x,
            a); // store as central vertical coordinate
...
InitCheepCheep:
  ...
  a = M(Enemy_Y_Position + x);
  writeData(CheepCheepOrigYPos + x,
            a); // save original vertical coordinate here
  goto Return;
...
SetupLakitu:
  ...
  JSR(InitHorizFlySwimEnemy,
      277);       // set $03 as bounding box, set other attributes
...
CreateSpiny:
  ...
  if (!z)
    goto ExLSHand;
  a = M(Enemy_PageLoc +
        y); // store horizontal coordinates (high and low) of lakitu
  writeData(Enemy_PageLoc + x,
            a); // into the coordinates of the spiny we're going to create
...
DifLoop: // get three values and save them
  ...
  JSR(PlayerLakituDiff,
      280);              // move enemy, change direction, get value - difference
  y = M(Player_X_Speed); // check player's horizontal speed
  compare(y, 0x08);
  if (c)
...
SetSpSpd: // set bounding box control, init attributes, lose contents of A
  JSR(SmallBBox, 281);
  y = 0x02;
  writeData(Enemy_X_Speed + x,
            a);     // set horizontal speed to zero because previous contents
...
InitFlyingCheepCheep:
  ...
  if (!z)
    goto ChpChpEx;
  JSR(SmallBBox,
      283); // jump to set bounding box size $09 and init other values
  a = M(PseudoRandomBitReg + 1 + x);
...
GSeed: // save to stack
  ...
  a = M(PseudoRandomBitReg + 1 + x);
  a &= BOOST_BINARY(
      00000011); // if neither of the last two bits of second LSFR set,
  if (z)
...
RSeed: // get value from stack we saved earlier
  ...
  writeData(Enemy_MovingDir + x, a);
  a = M(Player_X_Speed); // if player moving left or right, branch ahead of this
                         // part
  if (!z)
    goto D2XPos1;
...
D2XPos1: // get first LSFR or third LSFR lower nybble again
  ...
  a = M(Player_X_Position); // get player's horizontal position
  c = 0;
  a += M(FlyCCXPositionData +
         y); // if d1 set, add value obtained from pseudorandom offset
...
D2XPos2: // get player's horizontal position
  ...
  c = 1;
  a -= M(FlyCCXPositionData +
         y); // if d1 not set, subtract value obtained from pseudorandom
  writeData(Enemy_X_Position + x,
            a);          // offset and save as enemy's horizontal position
  a = M(Player_PageLoc); // get player's page location
  a -= 0x00;             // subtract borrow
...
FinCCSt: // save as enemy's page location
  ...
  a = 0xf8;
  writeData(Enemy_Y_Position + x,
            a); // put enemy below the screen, and we are done
  goto Return;
...
FSLoop: // increment one slot
  ...
  a = M(Enemy_PageLoc + x);
  writeData(Enemy_PageLoc + y,
            a);                // copy page location and horizontal coordinates
  ...
  a = M(Enemy_Y_Position + x);
  writeData(Enemy_Y_Position + y,
            a); // copy vertical coordinate from original to new
...
SetFrT: // set timer accordingly
  ...
  y = a;                                   // use as offset
  a = M(FlameYPosData +
        y); // load vertical position based on pseudorandom offset
...
SpawnFromMouth:
  ...
  compare(a, M(Enemy_Y_Position +
               x)); // compare value to flame's current vertical position
  if (!c)
...
FinishFlame:
  ...
  a >>= 1;
  writeData(Enemy_X_MoveForce + x,
            a);                  // initialize horizontal movement force, and
...
StarFChk:
  ...
  c = 0;
  a += M(Enemy_State +
         y); // add state of star flag object (possibly not necessary)
  y = a;     // use as offset
  pla();     // get saved horizontal coordinate of star flag - 48 pixels
  c = 0;
  a += M(FireworksXPosData +
         y); // add number based on offset of fireworks counter
  writeData(Enemy_X_Position + x,
            a); // store as the fireworks object horizontal coordinate
  a = M(0x00);
  ...
  writeData(Enemy_Y_Position + x,
            a); // and store as vertical coordinate for fireworks object
  a = 0x01;
...
InitPiranhaPlant:
  ...
  a >>= 1;
  writeData(Enemy_State + x,
            a); // initialize enemy state and what would normally
  writeData(PiranhaPlant_MoveFlag + x,
            a); // be used as vertical speed, but not in this case
  a = M(Enemy_Y_Position + x);
  writeData(PiranhaPlantDownYPos + x,
            a); // save original vertical coordinate here
  c = 1;
  a -= 0x18;
  writeData(PiranhaPlantUpYPos + x,
            a); // save original vertical coordinate - 24 pixels here
  a = 0x09;
...
SetYO: // save as top vertical position
  ...
  writeData(YPlatformCenterYPos + x,
            a); // save result as central vertical position
...
CommonSmallLift:
  ...
  a = 0x04;
  writeData(Enemy_BoundBoxCtrl + x,
            a); // set bounding box control for small platforms
  goto Return;
...
PosPlatform:
  ...
  a = M(Enemy_PageLoc + x);
  a += M(PlatPosDataHigh +
         y); // add or subtract page location depending on offset
...
ChkJH: // check jump timer
  ...
  if (!z)
    goto MoveHammerBroXDir;       // if hammer bro a little offscreen, skip to
                                  // movement code
...
SetHJ: // set vertical speed for jumping
  ...
  a &= M(PseudoRandomBitReg + 2 +
         x);                // and do bit-wise comparison with part of LSFR
...
HJump: // get jump length timer data using offset from before
  ...
  a = M(PseudoRandomBitReg + 1 + x);
  a |= BOOST_BINARY(
      11000000); // get contents of part of LSFR, set d7 and d6, then
...
Shimmy: // store horizontal speed
  ...
  JSR(PlayerEnemyDiff,
      326); // get horizontal difference between player and hammer bro
  if (n)
  ...
  a = 0xf8;
  writeData(Enemy_X_Speed + x,
            a); // otherwise, make the hammer bro walk left towards player
...
MoveNormalEnemy:
  ...
  compare(a, 0x03);
  if (c)
    goto ReviveStunned; // if enemy in states $03 or $04, skip ahead to yet
                        // another part
...
AddHS:
  ...
  pla();
  writeData(Enemy_X_Speed + x,
            a); // get old horizontal speed from stack and return to
...
MoveDefeatedEnemy:
  JSR(MoveD_EnemyVertically,
      329);                   // execute sub to move defeated enemy downwards
...
MoveJumpingEnemy:
  JSR(MoveJ_EnemyVertically,
      331);                   // do a sub to impose gravity on green paratroopa
...
ProcMoveRedPTroopa:
  ...
  ++M(Enemy_Y_Position +
      x); // otherwise increment red paratroopa's vertical position
...
MoveFlyGreenPTroopa:
  JSR(XMoveCntr_GreenPTroopa,
      332); // do sub to increment primary and secondary counters
  JSR(MoveWithXMCntrs,
      333); // do sub to move green paratroopa accordingly, and horizontally
...
DecSeXM: // put secondary counter in A
  ...
  --M(XMoveSecondaryCounter +
      x); // otherwise decrement secondary counter and leave
  goto Return;
...
MoveBloober:
  ...
  a &= M(BlooberBitmasks +
         y); // mask out bits in LSFR using bitmask loaded with offset
  if (!z)
...
FBLeft: // set left moving direction by default
  y = 0x02;
  JSR(PlayerEnemyDiff,
      335); // get horizontal difference between player and bloober
  if (!n)
...
BlooberSwim:
  JSR(ProcSwimmingB,
      336); // execute sub to make bloober swim characteristically
  ...
  writeData(Enemy_Y_Position + x,
            a); // otherwise, set new vertical position, make bloober swim
...
SwimX: // check moving direction
  ...
  writeData(Enemy_X_Position + x,
            a); // store result as new horizontal coordinate
  a = M(Enemy_PageLoc + x);
  a += 0x00;                       // add carry to page location
...
ChkNearPlayer:
  ...
  compare(
      a,
      M(Player_Y_Position)); // compare result with player's vertical coordinate
  if (!c)
...
MoveBulletBill:
  ...
  goto MoveJ_EnemyVertically; // otherwise jump to move defeated bullet bill
                              // downwards

NotDefB: // set bullet bill's horizontal speed
  a = 0xe8;
  writeData(Enemy_X_Speed + x,
            a);               // and move it accordingly (note: this bullet bill
  goto MoveEnemyHorizontally; // object occurs in frenzy object $17, not from
                              // cannons)
...
MoveSwimmingCheepCheep:
  ...
  goto MoveEnemySlowVert; // otherwise jump to move defeated cheep-cheep
                          // downwards
...
CCSwimUpwards:
  ...
  a = M(Enemy_Y_Position + x);       // get vertical coordinate
  a -= M(
      0x03); // subtract borrow to it plus enemy state to slowly move it upwards
  writeData(Enemy_Y_Position + x, a); // save as new vertical coordinate
  a = M(Enemy_Y_HighPos + x);
  a -= 0x00; // subtract borrow from page location
...
ProcFirebar:
  ...
  writeData(FirebarSpinState_High + x,
            a); // and store as new high byte of spinstate
...
SetupGFB: // save high byte of spinning thing, modified or otherwise
  ...
  JSR(GetFirebarPosition,
      340); // do a sub here (residual, too early to be used now)
...
DrawFbar: // load high byte of spinstate
  a = M(0xef);
  JSR(GetFirebarPosition,
      342); // get fireball position data depending on firebar part
  JSR(DrawFirebar_Collision,
      343);    // position it properly, draw it and do collision detection
  a = M(0x00); // check which firebar part
  compare(a, 0x04);
  if (!z)
    goto NextFbar;
  y = M(DuplicateObj_Offset); // if we arrive at fifth firebar part,
  a = M(Enemy_SprDataOffset +
        y);           // get offset from long firebar and load OAM data offset
...
AddHA: // add horizontal coordinate relative to screen to
  ...
  compare(a, M(Enemy_Rel_XPos)); // compare X coordinate of sprite to original X
                                 // of firebar
  if (c)
...
VAHandl: // if vertical relative coordinate offscreen,
  a = M(Enemy_Rel_YPos);
  compare(a,
          0xf8); // skip ahead of this part and write into sprite Y coordinate
  if (z)
    goto SetVFbr;
...
Chk2Ofs: // if value of $02 was set earlier for whatever reason,
  ...
  c = 0;
  a += M(FirebarYPos +
         y); // add value loaded with offset to player's vertical coordinate
...
BPGet: // load bg priority data and store (this is very likely
  ...
  writeData(Enemy_SprAttrib + x,
            a); // broken or residual code, value is overwritten before
...
MoveLakitu:
  ...
  goto MoveD_EnemyVertically; // otherwise jump to move defeated lakitu
                              // downwards
...
ChkLS: // if lakitu's enemy state not set at all,
  ...
  a = 0x00;
  writeData(LakituMoveDirection + x,
            a); // otherwise initialize moving direction to move to left
...
PlayerLakituDiff:
  ...
  JSR(PlayerEnemyDiff,
      349); // get horizontal difference between enemy and player
  if (!n)
...
ChkLakDif: // get low byte of horizontal difference
  ...
  compare(a, M(LakituMoveDirection +
               x)); // to what is being used as horizontal movement direction
  if (z)
    goto ChkPSpeed; // if moving toward the player, branch, do not alter
  a = M(LakituMoveDirection +
        x); // if moving to the left beyond maximum distance,
  if (z)
    goto SetLMovD;          // branch and alter without delay
  --M(LakituMoveSpeed + x); // decrement horizontal speed
  a = M(LakituMoveSpeed +
        x); // if horizontal speed not yet at zero, branch to leave
  if (!z)
    goto ExMoveLak;
...
SetLMovD: // set horizontal direction depending on horizontal
  a = y;
  writeData(LakituMoveDirection + x,
            a); // difference between enemy and player if necessary
...
SPixelLak: // subtract one for each pixel of horizontal difference
  ...
  --y;
  if (!n)
    goto SPixelLak; // branch until all pixels are subtracted, to adjust
                    // difference
...
BridgeCollapse:
  ...
  if (z)
    goto RemoveBridge;
  a &= BOOST_BINARY(
      01000000); // if bowser's state has d6 clear, skip to silence music
  if (z)
    goto SetM2;
...
RemoveBridge:
  ...
  a = M(BridgeCollapseData +
        y); // load low byte of name table address and store here
  writeData(0x04, a);
  ...
  writeData(Square2SoundQueue,
            a);         // queue while at the same time loading the brick
  a = Sfx_BrickShatter; // shatter sound into the noise sfx queue thus
  writeData(NoiseSoundQueue,
            a); // producing the unique sound of the bridge collapsing
  ...
  a = BOOST_BINARY(01000000);
  writeData(Enemy_State + x,
            a); // set bowser's state to one of defeated states (d6 set)
  a = Sfx_BowserFall;
...
B_FaceP: // if timer set here expired,
  ...
  JSR(PlayerEnemyDiff,
      355); // get horizontal difference between player and bowser,
  if (!n)
...
GetPRCmp: // get frame counter
  ...
  a = M(Enemy_X_Position + x);
  compare(a,
          M(BowserOrigXPos)); // if bowser not at original horizontal position,
  if (!z)
...
HammerChk: // if timer set here not expired yet, skip ahead to
  ...
  compare(a, World6);
  if (!c)
    goto SetHmrTmr; // if world 1-5, skip this part (not time to throw hammers
                    // yet)
  a = M(FrameCounter);
  ...
  JSR(SpawnHammerObj,
      357); // execute sub on every fourth frame to spawn misc object (hammer)
...
CopyFToR: // move bowser's rear object position value to A
  a = y;
  c = 0;
  a += M(Enemy_X_Position +
         x); // add to bowser's front object horizontal coordinate
  y = M(DuplicateObj_Offset); // get bowser's rear object offset
  writeData(Enemy_X_Position + y,
            a); // store A as bowser's rear horizontal coordinate
  a = M(Enemy_Y_Position + x);
...
ProcessBowserHalf:
  ...
  JSR(RunRetainerObj, 362); // to get offscreen bits, relative position and draw
                            // bowser (finally!)
  a = M(Enemy_State + x);
  if (!z)
...
SFlmX: // store value here
  ...
  compare(a, M(FlameYPosData +
               y)); // compare against coordinate data using $0417,x as offset
  if (z)
    goto SetGfxF; // if equal, branch and do not modify coordinate
  c = 0;
  a += M(Enemy_Y_MoveForce +
         x); // otherwise add value here to coordinate and store
...
DrawFlameLoop:
  ...
  a = M(0x00);
  writeData(Sprite_Tilenumber + y,
            a); // write current tile number into OAM data
  ...
  a = M(Enemy_Rel_XPos);
  writeData(Sprite_X_Position + y,
            a); // write X relative coordinate of current enemy object
  c = 0;
  a += 0x08;
  ...
  writeData(Sprite_Y_Position + 12 + y,
            a); // residual since flame is only made of three sprites
...
RunFireworks:
  ...
  if (c)
    goto FireworksSoundScore; // if at a certain point, branch to kill this
                              // object

SetupExpl: // get relative coordinates of explosion
  JSR(RelativeEnemyPosition, 366);
  a = M(Enemy_Rel_YPos); // copy relative coordinates
  writeData(Fireball_Rel_YPos,
            a);          // from the enemy object to the fireball object
  ...
  JSR(DrawExplosion_Fireworks,
      367); // do a sub to draw the explosion then leave
  goto Return;
...
SetFWC: // set fireworks counter here
  ...
  writeData(Enemy_State + x,
            y); // set whatever state we have in star flag object
...
AwardGameTimerPoints:
  ...
  a |= M(GameTimerDisplay + 2);
  if (z)
    goto IncrementSFTask1; // if no time left on game timer at all, branch to
                           // next task
  a = M(FrameCounter);
...
SetoffF: // check fireworks counter
  ...
  a = Fireworks;
  writeData(EnemyFrenzyBuffer,
            a); // otherwise set fireworks object in frenzy queue
...
MovePiranhaPlant:
  ...
  if (!n)
    goto ChkPlayerNearPipe; // piranha plant, and branch if enemy to right of
                            // player
...
SetupToMovePPlant:
  a = M(PiranhaPlantDownYPos +
        x); // get original vertical coordinate (lowest point)
  y = M(PiranhaPlant_Y_Speed + x); // get vertical speed
  if (!n)
    goto RiseFallPiranhaPlant; // branch if moving downwards
  a = M(PiranhaPlantUpYPos +
        x); // otherwise get other vertical coordinate (highest point)
...
RiseFallPiranhaPlant:
  ...
  a >>= 1;
  if (!c)
    goto PutinPipe;    // branch to leave if d0 set (execute code every other
                       // frame)
  ...
  a = 0x40;
  writeData(EnemyFrameTimer + x,
            a); // set timer to delay piranha plant movement

PutinPipe:
  a = BOOST_BINARY(00100000); // set background priority bit in sprite
  writeData(Enemy_SprAttrib + x,
            a); // attributes to give illusion of being inside pipe
  goto Return;  // then leave
...
FirebarSpin:
  ...
  if (!z)
    goto SpinCounterClockwise; // if moving counter-clockwise, branch to other
                               // part
  ...
  writeData(FirebarSpinState_Low + x, a);
  a = M(FirebarSpinState_High +
        x); // add carry to what would normally be the vertical speed
  a += 0x00;
  goto Return;
...
ChkForFall:
  ...
  writeData(Enemy_Y_Position + x,
            a);       // of current platform and branch elsewhere
...
ChkOtherForFall:
  compare(a, M(Enemy_Y_Position +
               y)); // check if other platform is above a certain point
  if (!c)
...
DoOtherPlatform:
  ...
  c = 0;
  a +=
      M(Enemy_Y_Position + y); // add difference to vertical coordinate of other
  writeData(Enemy_Y_Position + y,
            a); // platform to move it in the opposite direction
...
DrawEraseRope:
  ...
  JSR(SetupPlatformRope,
      378);    // do a sub to figure out where to put new bg tiles
...
OtherRope:
  ...
  JSR(SetupPlatformRope,
      379);    // do sub again to figure out where to put bg tiles
  a = M(0x01); // write name table address to vram buffer
...
InitPlatformFall:
  ...
  a = M(Player_Rel_XPos);
  writeData(FloateyNum_X_Pos + x,
            a); // put floatey number coordinates where player is
  a = M(Player_Y_Position);
...
PlatformFall:
  ...
  x = M(ObjectOffset);
  a = M(PlatformCollisionFlag +
        x); // if player not standing on either platform,
  if (n)
...
YMovingPlatform:
  ...
  writeData(Enemy_YMF_Dummy + x, a); // initialize dummy variable
  a = M(Enemy_Y_Position + x);
  compare(a, M(YPlatformTopYPos +
               x)); // if current vertical position => top position, branch
  if (c)
...
ChkYCenterPos:
  a = M(Enemy_Y_Position +
        x); // if current vertical position < central position, branch
...
XMovingPlatform:
  ...
  JSR(MoveWithXMCntrs,
      390); // do a sub to move platform accordingly, and return value
...
SetPVar: // save result to player's page location
  writeData(Player_PageLoc, a);
  writeData(Platform_X_Scroll,
            y); // put saved value from second sub here to be used later
  JSR(PositionPlayerOnVPlat,
      391); // position player vertically and appropriately
...
DropPlatform:
  a = M(PlatformCollisionFlag +
        x); // if no collision between platform and player
  if (n)
    goto ExDPl; // occurred, just leave without moving anything
  JSR(MoveDropPlatform,
      392); // otherwise do a sub to move platform down very quickly
...
RightPlatform:
  JSR(MoveEnemyHorizontally,
      394);           // move platform with current horizontal speed, if any
  writeData(0x00, a); // store saved value here (residual code)
  a = M(PlatformCollisionFlag +
        x); // check collision flag, if no collision between player
  if (n)
    goto ExRPl; // and platform, branch ahead, leave speed unaltered
  a = 0x10;
  writeData(Enemy_X_Speed + x,
            a); // otherwise set new speed (gets moving if motionless)
  JSR(PositionPlayerOnHPlat,
      395); // use saved value from earlier sub to position player
...
MoveLargeLiftPlat:
  JSR(MoveLiftPlatforms,
      396);            // execute common to all large and small lift platforms
  goto ChkYPCollision; // branch to position player correctly

MoveSmallPlatform:
  JSR(MoveLiftPlatforms,
      397); // execute common to all large and small lift platforms
...
ChkSmallPlatCollision:
  a = M(PlatformCollisionFlag +
        x); // get bounding box counter saved in collision flag
  if (z)
...
OffscreenBoundsCheck:
  ...
  if (!z)
    goto ExtendLB; // these two will be erased sooner than others if too far
                   // left
...
ExtendLB: // subtract 72 pixels regardless of enemy object
  ...
  a = M(ScreenRight_X_Pos); // add 72 pixels to the right side horizontal
                            // coordinate
  a += 0x48;
  ...
  a = M(Enemy_X_Position +
        x); // compare horizontal coordinate of the enemy object
  compare(a,
          M(0x01)); // to modified horizontal left edge coordinate to get carry
  a = M(Enemy_PageLoc + x);
  ...
  a = M(Enemy_X_Position +
        x); // compare horizontal coordinate of the enemy object
  compare(a,
          M(0x03)); // to modified horizontal right edge coordinate to get carry
  a = M(Enemy_PageLoc + x);
  a -= M(0x02); // then subtract it from the page coordinate of the enemy object
  if (n)
    goto ExScrnBd; // if enemy object is on the screen, leave, do not erase
                   // enemy
  a = M(Enemy_State +
        x); // if at this point, enemy is offscreen to the right, so check
...
FireballEnemyCollision:
  ...
  if (c)
    goto ExitFBallEnemy; // branch to leave if set (do routine every other
                         // frame)
  a = x;
...
NoFToECol: // pull fireball offset from stack
  ...
  --x;         // decrement it
  if (!n)
    goto FireballEnemyCDLoop; // loop back until collision detection done on all
                              // enemies
...
ChkBuzzyBeetle:
  ...
  compare(a, Bowser); // check for bowser one more time (necessary if d7 of flag
                      // was clear)
  if (!z)
...
HurtBowser:
  ...
  JSR(InitVStf,
      403); // otherwise do sub to init vertical speed and movement force
  writeData(Enemy_X_Speed + x, a); // initialize horizontal speed
  writeData(EnemyFrenzyBuffer, a); // init enemy frenzy buffer
  a = 0xfe;
  writeData(Enemy_Y_Speed + x,
            a); // set vertical speed to make defeated bowser jump a little
...
PlayerHammerCollision:
  ...
  a += 0x01;
  writeData(Misc_X_Speed + x,
            a); // set to send hammer flying the opposite direction
...
PlayerEnemyCollision:
  ...
  a = M(EnemyOffscrBitsMasked +
        x); // if current enemy is offscreen by any amount,
  if (!z)
  ...
  JSR(GetEnemyBoundBoxOfs,
      412); // get bounding box offset for current enemy object
...
HandlePECollisions:
  ...
  a = M(KickedShellXSpdData +
        y); // load and set horizontal speed data with offset
  writeData(Enemy_X_Speed + x, a);
  ...
  a = M(KickedShellPtsData +
        y); // otherwise, set points based on proximity to timer expiration
...
ChkForPlayerInjury:
  ...
  if (!z)
    goto EnemyStomped; // or not at all, and branch elsewhere if moving
                       // downwards
...
ChkInj: // branch if enemy object < $07
  ...
  a += 0x0c;
  compare(a, M(Enemy_Y_Position +
               x)); // compare modified player's position to enemy's position
  if (!c)
    goto EnemyStomped; // branch if this player's position above (less than)
                       // enemy's
...
ChkETmrs: // check stomp timer
  ...
  a = M(Player_Rel_XPos);
  compare(a, M(Enemy_Rel_XPos)); // if player's relative position to the left of
                                 // enemy's
  if (!c)
...
KillPlayer:
  writeData(Player_X_Speed,
            x); // halt player's horizontal movement by initializing speed
  ++x;
...
EnemyStomped:
  ...
  if (z)
    goto EnemyStompedPts;
  compare(a,
          Podoboo); // branch for podoboo (this branch is logically impossible
  if (z)
...
EnemyStompedPts:
  ...
  pla();
  writeData(Enemy_MovingDir + x,
            a); // return enemy movement direction from stack
  a = BOOST_BINARY(00100000);
...
ChkForDemoteKoopa:
  ...
  writeData(Enemy_X_Speed + x,
            a); // set appropriate moving speed based on direction
...
HandleStompedShellE:
  ...
  writeData(EnemyIntervalTimer + x,
            a); // set as enemy timer to revive stomped enemy
...
EnemyFacePlayer:
  y = 0x01; // set to move right by default
  JSR(PlayerEnemyDiff,
      425); // get horizontal difference between player and enemy
  if (!n)
...
SetupFloateyNumber:
  writeData(FloateyNum_Control + x,
            a); // set number of points control for floatey numbers
  a = 0x30;
...
EnemiesCollision:
  ...
  if (z)
    goto ExitECRoutine;
  a = M(EnemyOffscrBitsMasked +
        x); // if masked offscreen bits nonzero, branch to leave
  if (!z)
    goto ExitECRoutine;
  JSR(GetEnemyBoundBoxOfs,
      426); // otherwise, do sub, get appropriate bounding box offset for
...
ECLoop: // save enemy object buffer offset for second enemy here
  ...
  JSR(SprObjectCollisionCore,
      427);            // do collision detection using the two enemies here
...
ShellCollisions:
  ...
  JSR(SetupFloateyNumber,
      432);            // award appropriate number of points for second enemy
...
ProcSecondEnemyColl:
  ...
  x = M(ObjectOffset);
  JSR(SetupFloateyNumber,
      434);    // award appropriate number of points for first enemy
...
RXSpd: // load horizontal speed
  ...
  writeData(Enemy_MovingDir + x,
            a); // thus effectively turning the enemy around
...
LargePlatformCollision:
  ...
  JSR(ChkForPlayerC_LargeP,
      436); // perform code with state offset, then original offset, in X

ChkForPlayerC_LargeP:
  JSR(CheckPlayerVertical,
      437); // figure out if player is below a certain point
  if (c)
...
SmallPlatformCollision:
  ...
  writeData(PlatformCollisionFlag + x,
            a); // otherwise initialize collision flag
  JSR(CheckPlayerVertical,
      441); // do a sub to see if player is below a certain point
  if (c)
...
ProcLPlatCollisions:
  ...
  a = M(
      Player_Y_Speed); // check to see if player's vertical speed is moving down
  if (!n)
...
ChkForTopCollision:
  ...
  compare(a, 0x06);
  if (c)
    goto PlatformSideCollisions; // if difference not close enough, skip all of
                                 // this
  a = M(Player_Y_Speed);
  if (n)
    goto PlatformSideCollisions; // if player's vertical speed moving upwards,
                                 // skip this
  a = M(0x00);                   // get saved bounding box counter from earlier
  y = M(Enemy_ID + x);
  compare(y, 0x2b); // if either of the two small platform objects are found,
  if (z)
    goto SetCollisionFlag; // regardless of which one, branch to use bounding
                           // box counter
...
SetCollisionFlag:
  x = M(ObjectOffset); // get enemy object buffer offset
  writeData(PlatformCollisionFlag + x,
            a); // save either bounding box counter or enemy offset here
  a = 0x00;
...
PlatformSideCollisions:
  ...
  a = M(BoundingBox_DR_XPos); // get difference by subtracting platform's left
                              // edge
  ...
  a = M(BoundingBox_DR_XPos +
        y); // get difference by subtracting player's left edge
...
Skip_8:
  ...
  a = 0x00;
  writeData(Player_Y_Speed,
            a); // initialize vertical speed and low byte of force
...
HeadChk: // get player's vertical coordinate
  a = M(Player_Y_Position);
  compare(a, M(PlayerBGUpperExtent +
               x)); // compare with upper extent value based on offset
  if (!c)
    goto DoFootCheck; // if player is too high, skip this part
  JSR(BlockBufferColli_Head,
      445); // do player-to-bg collision detection on top of
  if (z)
    goto DoFootCheck; // player, and branch if nothing above player's head
  JSR(CheckForCoinMTiles,
      446); // check to see if player touched coin with their head
  if (c)
...
DoFootCheck:
  ...
  if (c)
    goto DoPlayerSideCheck; // if player is too far down on screen, skip all of
                            // this
  JSR(BlockBufferColli_Feet,
      449); // do player-to-bg collision detection on bottom left of player
  JSR(CheckForCoinMTiles,
      450); // check to see if player touched coin with their left foot
  if (c)
    goto AwardTouchedCoin; // if so, branch to some other part of code
  pha();                   // save bottom left metatile to stack
  JSR(BlockBufferColli_Feet,
      451); // do player-to-bg collision detection on bottom right of player
  ...
  JSR(CheckForCoinMTiles,
      452); // check to see if player touched coin with their right foot
  if (!c)
    goto ChkFootMTile; // if not, skip unconditional jump and continue code

AwardTouchedCoin:
  goto HandleCoinMetatile; // follow the code to erase coin and award to player
                           // 1 coin

ChkFootMTile:
  JSR(CheckForClimbMTiles,
      453); // check to see if player landed on climbable metatiles
  if (c)
...
LandPlyr: // do sub to check for jumpspring metatiles and deal with it
  ...
  a = 0xf0;
  a &= M(
      Player_Y_Position); // mask out lower nybble of player's vertical position
  writeData(Player_Y_Position,
            a); // and store as new vertical position to land player properly
  ...
  writeData(Player_Y_MoveForce,
            a); // movement force to stop player's vertical movement
...
SideCheckLoop:
  ...
  if (!c)
    goto BHalf; // if player is in status bar area, branch ahead to skip this
                // part
  compare(a, 0xe4);
  if (c)
    goto ExSCH; // branch to leave if player is too far down
  JSR(BlockBufferColli_Side,
      457); // do player-to-bg collision detection on one half of player
  if (z)
  ...
  JSR(CheckForClimbMTiles,
      458); // do sub to see if player bumped into anything climbable
  if (!c)
...
BHalf: // load block adder offset
  ...
  JSR(BlockBufferColli_Side,
      459); // do player-to-bg collision detection on other half of player
  if (!z)
...
ContSChk: // check to see if player touched coin
  ...
  if (c)
    goto HandleCoinMetatile; // if so, execute code to erase coin and award to
                             // player 1 coin
...
PlyrPipe:
  a |= BOOST_BINARY(00100000);
  writeData(Player_SprAttrib,
            a); // set background priority bit in player attributes
  a = M(Player_X_Position);
  a &= BOOST_BINARY(
      00001111); // get lower nybble of player's horizontal coordinate
  if (z)
...
ChkGERtn: // get number of game engine routine running
  ...
  a = 0x02;
  writeData(GameEngineSubroutine,
            a); // otherwise set sideways pipe entry routine to run
  goto Return;  // and leave
...
HandleAxeMetatile:
  ...
  a = 0x18;
  writeData(Player_X_Speed,
            a); // set horizontal speed and continue to erase axe metatile
...
FlagpoleCollision:
  ...
  a >>= 1;
  writeData(FlagpoleSoundQueue,
            a); // load flagpole sound into flagpole sound queue
  x = 0x04;     // start at end of vertical coordinate data
  a = M(Player_Y_Position);
  writeData(FlagpoleCollisionYPos,
            a); // store player's vertical coordinate here to be used later

ChkFlagpoleYPosLoop:
  compare(a, M(FlagpoleYPosData +
               x)); // compare with current vertical coordinate data
  if (c)
    goto MtchF; // if player's => current, branch to use current offset
  --x;          // otherwise decrement offset to use
  if (!z)
    goto ChkFlagpoleYPosLoop; // do this until all data is checked (use last one
                              // if all checked)
...
VineCollision:
  ...
  a = 0x01;
  writeData(GameEngineSubroutine,
            a); // otherwise set to run autoclimb routine next frame
...
PutPlayerOnVine:
  ...
  compare(a, 0x10);
  if (c)
    goto SetVXPl; // if 16 or more pixels difference, do not alter facing
                  // direction
  a = 0x02;
...
ChkForLandJumpSpring:
  JSR(ChkJumpspringMetatiles,
      467); // do sub to check if player landed on jumpspring
  if (!c)
    goto ExCJSp; // if carry not set, jumpspring not found, therefore leave
  a = 0x70;
  writeData(VerticalForce,
            a); // otherwise set vertical movement force for player
  a = 0xf9;
  ...
  a >>= 1;
  writeData(JumpspringAnimCtrl,
            a); // set jumpspring animation control to start animating
...
HandlePipeEntry:
  ...
  a = 0x03;
  writeData(GameEngineSubroutine,
            a); // set to run vertical pipe entry routine on next frame
  a = Sfx_PipeDown_Injury;
  writeData(Square1SoundQueue, a); // load pipedown/injury sound
  a = BOOST_BINARY(00100000);
  writeData(Player_SprAttrib,
            a);           // set background priority bit in player's attributes
  ...
  compare(a, 0x60);
  if (!c)
    goto GetWNum; // if player at left, not near middle, use offset and skip
                  // ahead
  ++x;            // otherwise increment for middle pipe
  compare(a, 0xa0);
  if (!c)
    goto GetWNum; // if player at middle, but not too far right, use offset and
                  // skip
...
GetWNum: // get warp zone numbers
  ...
  x = M(WorldAddrOffsets +
        y); // get offset to where this world's area offsets are
  a = M(AreaAddrOffsets + x); // get area offset based on world offset
  writeData(AreaPointer,
            a); // store area offset here to be used to change areas
  a = Silence;
  writeData(EventMusicQueue, a); // silence music
  a = 0x00;
  writeData(EntrancePage, a); // initialize starting page number
  writeData(AreaNumber,
            a); // initialize area number used for area address offset
...
CheckForSolidMTiles:
  ...
  compare(a, M(SolidMTileUpperExt +
               x)); // compare current metatile with solid metatiles
  goto Return;
...
CheckForClimbMTiles:
  ...
  compare(a, M(ClimbMTileUpperExt +
               x)); // compare current metatile with climbable metatiles
  goto Return;
...
HandleEToBGCollision:
  ...
  if (z)
    goto NoEToBGCollision; // if blank $26, coins, or hidden blocks, jump, enemy
                           // falls through
  compare(a, 0x23);
  if (!z)
    goto LandEnemyProperly; // check for blank metatile $23 and branch if not
                            // found
  ...
  compare(a,
          Goomba); // if enemy object not goomba, branch ahead of this routine
  if (!z)
    goto GiveOEPoints;
...
ChkToStunEnemies:
  ...
  compare(a, 0x0a); // modified if not any of those values, note that piranha
                    // plant will
  if (!c)
    goto Demote; // always fail this test because A will still have vertical
  compare(a, PiranhaPlant); // coordinate from previous addition, also these
                            // comparisons
  if (!c)
...
SetStun: // load enemy state
  ...
  --M(Enemy_Y_Position + x);
  --M(Enemy_Y_Position +
      x); // subtract two pixels from enemy's vertical position
  a = M(Enemy_ID + x);
...
SetNotW: // set vertical speed now
  ...
  y = 0x01;
  JSR(PlayerEnemyDiff,
      475); // get horizontal difference between player and enemy object
  if (!n)
...
LandEnemyProperly:
  ...
  if (c)
    goto ChkForRedKoopa; // branch if lower nybble in range of $0d-$0f before
                         // subtract
  a = M(Enemy_State + x);
...
ProcEnemyDirection:
  ...
  a = 0x01;
  writeData(Enemy_MovingDir + x,
            a); // send enemy moving to the right by default
  a = 0x08;
...
InvtD: // load 1 for enemy to face the left (inverted here)
  y = 0x01;
  JSR(PlayerEnemyDiff,
      477); // get horizontal difference between player and enemy
  if (!n)
...
CNwCDir:
  a = y;
  compare(a, M(Enemy_MovingDir +
               x)); // compare direction in A with current direction in memory
  if (!z)
    goto LandEnemyInitState;
  JSR(ChkForBump_HammerBroJ,
      478); // if equal, not facing in correct dir, do sub to turn around
...
LandEnemyInitState:
  ...
  writeData(Enemy_State + x,
            a); // note this will also turn spiny's egg into spiny
  goto Return;
...
ChkForBump_HammerBroJ:
  ...
  writeData(Square1SoundQueue,
            a); // sound will never be played if branching from ChkForRedKoopa
...
EnemyLanding:
  ...
  a |= BOOST_BINARY(
      00001000); // set d3, then store, probably used to set enemy object
...
UnderHammerBro:
  ...
  a = M(Enemy_State + x);
  a &= BOOST_BINARY(
      10001000); // save d7 and d3 from enemy state, nullify other bits
  writeData(Enemy_State + x, a); // and store
  JSR(EnemyLanding,
      489);              // modify vertical coordinate, speed and something else
...
FireballBGCollision:
  ...
  compare(a, 0x18);
  if (!c)
    goto ClearBounceFlag; // if within the status bar area of the screen, branch
                          // ahead
  JSR(BlockBufferChk_FBall,
      490); // do fireball to background collision detection on bottom of it
  if (z)
  ...
  a = M(Fireball_Y_Speed +
        x); // if fireball's vertical speed set to move upwards,
  if (n)
  ...
  if (!z)
    goto InitFireballExplode; // branch to set exploding bit in fireball's state
  a = 0xfd;
  writeData(Fireball_Y_Speed + x,
            a); // otherwise set vertical speed to move upwards (give it bounce)
  a = 0x01;
...
LargePlatformBoundBox:
  ...
  JSR(GetXOffscreenBits,
      493); // then jump directly to the sub for horizontal offscreen bits
  --x;      // decrement to return to original offset
  compare(a, 0xfe); // if completely offscreen, branch to put entire bounding
  if (c)
    goto MoveBoundBoxOffscreen; // box offscreen, otherwise start getting
                                // coordinates
...
SetupEOffsetFBBox:
  ...
  JSR(BoundingBoxCore,
      494); // do a sub to get the coordinates of the bounding box
  goto CheckRightScreenBBox; // jump to handle offscreen coordinates of bounding
                             // box
...
MoveBoundBoxOffscreen:
  ...
  a = 0xff;
  writeData(EnemyBoundingBoxCoord + y,
            a); // load value into four locations here and leave
...
BoundingBoxCore:
  ...
  a = M(0x01);
  c = 0;
  a += M(BoundBoxCtrlData + 2 +
         x); // add the third number in the bounding box data to the
  writeData(BoundingBox_LR_Corner + y,
            a); // relative horizontal coordinate and store
  ...
  c = 0;
  a += M(BoundBoxCtrlData + 2 +
         x); // add the fourth number to the relative vertical coordinate
...
CheckRightScreenBBox:
  ...
  writeData(0x02, a);
  a = M(
      ScreenLeft_PageLoc); // add carry to page location of left side of screen
  ...
  if (!c)
    goto CheckLeftScreenBBox; // if object is on the left side of the screen,
                              // branch
  a = M(BoundingBox_DR_XPos +
        y); // check right-side edge of bounding box for offscreen
  if (n)
  ...
  x = M(BoundingBox_UL_XPos +
        y); // check left-side edge of bounding box for offscreen
  if (n)
...
SprObjectCollisionCore:
  ...
  writeData(
      0x07,
      a); // save value 1 here as counter, compare horizontal coordinates first

CollisionCoreLoop:
  a = M(BoundingBox_UL_Corner + y); // compare left/top coordinates
  compare(a, M(BoundingBox_UL_Corner +
               x)); // of first and second objects' bounding boxes
  if (c)
    goto FirstBoxGreater; // if first left/top => second, branch
  compare(a, M(BoundingBox_LR_Corner +
               x)); // otherwise compare to right/bottom of second
  if (!c)
    goto SecondBoxVerticalChk; // if first left/top < second right/bottom,
                               // branch elsewhere
  if (z)
    goto CollisionFound; // if somehow equal, collision, thus branch
  a = M(BoundingBox_LR_Corner +
        y); // if somehow greater, check to see if bottom of
  compare(a, M(BoundingBox_UL_Corner +
               y)); // first object's bounding box is greater than its top
  if (!c)
    goto CollisionFound; // if somehow less, vertical wrap collision, thus
                         // branch
  compare(a, M(BoundingBox_UL_Corner +
               x)); // otherwise compare bottom of first bounding box to the top
  if (c)
    goto CollisionFound; // of second box, and if equal or greater, collision,
                         // thus branch
...
SecondBoxVerticalChk:
  a = M(BoundingBox_LR_Corner +
        x); // check to see if the vertical bottom of the box
  compare(a, M(BoundingBox_UL_Corner + x)); // is greater than the vertical top
  if (!c)
    goto CollisionFound; // if somehow less, vertical wrap collision, thus
                         // branch
  a = M(BoundingBox_LR_Corner +
        y); // otherwise compare horizontal right or vertical bottom
  compare(
      a,
      M(BoundingBox_UL_Corner +
        x)); // of first box with horizontal left or vertical top of second box
  if (c)
...
FirstBoxGreater:
  compare(
      a,
      M(BoundingBox_UL_Corner +
        x)); // compare first and second box horizontal left/vertical top again
  if (z)
    goto CollisionFound; // if first coordinate = second, collision, thus branch
  compare(a, M(BoundingBox_LR_Corner +
               x)); // if not, compare with second object right or bottom edge
  if (!c)
    goto CollisionFound; // if left/top of first less than or equal to
                         // right/bottom of second
  if (z)
    goto CollisionFound;                    // then collision, thus branch
  compare(a, M(BoundingBox_LR_Corner + y)); // otherwise check to see if top of
                                            // first box is greater than bottom
  if (!c)
    goto NoCollisionFound; // if less than or equal, no collision, branch to end
  if (z)
    goto NoCollisionFound;
  a = M(BoundingBox_LR_Corner +
        y); // otherwise compare bottom of first to top of second
  compare(
      a,
      M(BoundingBox_UL_Corner +
        x)); // if bottom of first is greater than top of second, vertical wrap
  if (c)
    goto CollisionFound; // collision, and branch, otherwise, proceed onwards
                         // here

NoCollisionFound:
  c = 0;       // clear carry, then load value set earlier, then leave
  y = M(0x06); // like previous ones, if horizontal coordinates do not collide,
               // we do
...
CollisionFound:
  ...
  c = 1; // otherwise we already did both sets, therefore collision, so set
         // carry
...
ResidualMiscObjectCode:
  ...
  goto ResJmpM; // probably used in early stages to do misc to bg collision
                // detection
...
DrawVine:
  ...
  JSR(SixSpriteStacker,
      497);              // stack six sprites on top of each other vertically
  a = M(Enemy_Rel_XPos); // get relative horizontal coordinate
  writeData(Sprite_X_Position + y,
            a); // store in first, third and fifth sprites
  ...
  writeData(Sprite_X_Position + 4 + y,
            a); // to give characteristic staggered vine shape to
  ...
  writeData(Sprite_Attributes + 4 + y,
            a); // for second, fourth and sixth sprites
...
GetHPose: // get frame counter
  ...
  a >>= 1;
  a &= BOOST_BINARY(
      00000011); // mask out all but d1-d0 (changes every four frames)
...
RenderH: // get relative vertical coordinate
  ...
  writeData(Sprite_Y_Position + y,
            a); // store as sprite Y coordinate for first sprite
  c = 0;
  a += M(SecondSprYPos + x); // add second sprite vertical adder based on offset
  writeData(Sprite_Y_Position + 4 + y,
            a);         // store as sprite Y coordinate for second sprite
  a = M(Misc_Rel_XPos); // get relative horizontal coordinate
  c = 0;
  a += M(FirstSprXPos + x); // add first sprite horizontal adder based on offset
  writeData(Sprite_X_Position + y,
            a); // store as sprite X coordinate for first sprite
  c = 0;
  a += M(SecondSprXPos +
         x); // add second sprite horizontal adder based on offset
  writeData(Sprite_X_Position + 4 + y,
            a); // store as sprite X coordinate for second sprite
  a = M(FirstSprTilenum + x);
  writeData(Sprite_Tilenumber + y,
            a); // get and store tile number of first sprite
  a = M(SecondSprTilenum + x);
  writeData(Sprite_Tilenumber + 4 + y,
            a); // get and store tile number of second sprite
  a = M(HammerSprAttrib + x);
  writeData(Sprite_Attributes + y, a); // get and store attribute bytes for both
  writeData(Sprite_Attributes + 4 + y,
            a);        // note in this case they use the same data
  x = M(ObjectOffset); // get misc object offset
...
FlagpoleGfxHandler:
  ...
  writeData(Sprite_X_Position + 4 + y,
            a); // as X coordinate for second and third sprites
  writeData(Sprite_X_Position + 8 + y, a);
  c = 0;
  ...
  writeData(Sprite_Attributes + y,
            a); // set attribute bytes for all three sprites
...
DrawLargePlatform:
  ...
  JSR(SixSpriteStacker,
      501); // store X coordinates using A as base, stack horizontally
  x = M(ObjectOffset);
  a = M(Enemy_Y_Position + x); // get vertical coordinate
...
SetLast2Platform:
  y = M(Enemy_SprDataOffset + x); // get OAM data offset
  writeData(Sprite_Y_Position + 16 + y,
            a); // store vertical coordinate or offscreen
  writeData(Sprite_Y_Position + 20 + y,
            a); // coordinate into last two sprites as Y coordinate
...
SChk6: // get bits from stack
  ...
  a = 0xf8;
  writeData(Sprite_Y_Position + 20 + y,
            a); // if d2 was set, move sixth sprite offscreen
...
SLChk: // check d7 of offscreen bits
  ...
  JSR(MoveSixSpritesOffscreen,
      506); // otherwise branch to move all sprites offscreen
...
NotRsNum: // get vertical coordinate
  ...
  a += 0x08; // add eight pixels
  writeData(Sprite_X_Position + 4 + y,
            a); // store as X coordinate for second sprite
  a = 0x02;
...
JCoinGfxHandler:
  ...
  a += 0x08; // add eight pixels
  writeData(Sprite_Y_Position + 4 + y,
            a);         // store as Y coordinate for second sprite
  a = M(Misc_Rel_XPos); // get relative horizontal coordinate
  writeData(Sprite_X_Position + y, a);
  writeData(Sprite_X_Position + 4 + y,
            a);        // store as X coordinate for first and second sprites
  a = M(FrameCounter); // get frame counter
  ...
  a = 0x82;
  writeData(Sprite_Attributes + 4 + y,
            a);        // set attribute byte with vertical flip in second sprite
  x = M(ObjectOffset); // get misc object offset
...
PUpDrawLoop:
  ...
  a &= BOOST_BINARY(
      00000011); // mask out all but d1 and d0 (previously d2 and d1)
  a |= M(Enemy_SprAttrib + 5); // add background priority bit if any set
  writeData(Sprite_Attributes + y,
            a); // set as new palette bits for top left and
  writeData(Sprite_Attributes + 4 + y,
            a); // top right sprites for fire flower and star
  x = M(0x00);
  --x; // check power-up type for fire flower
  if (z)
    goto FlipPUpRightSide; // if found, skip this part
  writeData(Sprite_Attributes + 8 + y,
            a); // otherwise set new palette bits  for bottom left
  writeData(Sprite_Attributes + 12 + y,
            a); // and bottom right sprites as well for star only
...
FlipPUpRightSide:
  ...
  a |=
      BOOST_BINARY(01000000); // set horizontal flip bit for bottom right sprite
  writeData(Sprite_Attributes + 12 + y,
            a); // note these are only done for fire flower and star power-ups
...
GmbaAnim: // check for d5 set in enemy object state
  ...
  a = M(0x03);
  a ^= BOOST_BINARY(
      00000011);      // invert bits to flip horizontally every eight frames
...
CheckBowserFront:
  ...
  compare(a, 0x01);
  if (!z)
    goto CheckBowserRear; // if not drawing front part, branch to draw the rear
                          // part
...
CheckRightSideUpShell:
  ...
  if (!z)
    goto CheckForHammerBro; // enemy state => $02 but not = $04, leave shell
                            // upside-down
...
CheckToAnimateEnemy:
  ...
  compare(a, 0x08);
  if (z)
    goto CheckDefeatedState; // branch if bullet bill (note both variants use
                             // $08 here)
  compare(a, Podoboo);
  if (z)
  ...
  if (!z)
    goto CheckForSecondFrame; // which uses different code here, branch if not
                              // found
...
CheckForSecondFrame:
  ...
  a &= M(EnemyAnimTimingBMask +
         y); // mask it (partly residual, one byte not ever used)
  if (!z)
...
ESRtnr: // check for princess/mushroom retainer object
  ...
  writeData(Sprite_Attributes + 20 + y,
            a); // note that palette bits were already set earlier
...
MirrorEnemyGfx:
  ...
  writeData(Sprite_Attributes + y,
            a); // save vertical flip, priority, and palette bits
  writeData(Sprite_Attributes + 8 + y,
            a); // in left sprite column of enemy object OAM data
  writeData(Sprite_Attributes + 16 + y, a);
...
EggExc: // set bits of right sprite column
  ...
  a |= BOOST_BINARY(01000000);
  writeData(Sprite_Attributes + 12 + y,
            a); // store with horizontal and vertical flip in
  writeData(Sprite_Attributes + 20 + y,
            a); // second and third row right sprites
...
CheckToMirrorLakitu:
  ...
  writeData(Sprite_Attributes + 12 + y,
            a); // otherwise set same for second row right sprite
  a &= BOOST_BINARY(10000001);
  writeData(Sprite_Attributes + 8 + y,
            a); // preserve vertical flip and palette bits for left sprite
  if (!c)
...
NVFLak: // get first row left sprite attributes
  ...
  writeData(Sprite_Attributes + 4 + y,
            a); // note that vertical flip is left as-is
...
CheckToMirrorJSpring:
  ...
  a = 0x82;
  writeData(Sprite_Attributes + 8 + y,
            a); // set vertical flip and palette bits of
  writeData(Sprite_Attributes + 16 + y, a); // second and third row left sprites
  a |= BOOST_BINARY(01000000);
  writeData(Sprite_Attributes + 12 + y,
            a); // set, in addition to those, horizontal flip
  writeData(Sprite_Attributes + 20 + y,
            a); // for second and third row right sprites
...
AllRowC: // get from stack once more
  ...
  JSR(MoveESprRowOffscreen,
      518); // move all sprites offscreen (A should be 0 by now)
  a = M(Enemy_ID + x);
  compare(a, Podoboo); // check enemy identifier for podoboo
  if (z)
    goto ExEGHandler; // skip this part if found, we do not want to erase
                      // podoboo!
  a = M(Enemy_Y_HighPos + x); // check high byte of vertical position
...
MoveESprColOffscreen:
  ...
  JSR(MoveColOffscreen,
      520); // move first and second row sprites in column offscreen
  writeData(Sprite_Data + 16 + y,
            a); // move third row sprite in column offscreen
  goto Return;
...
DBlkLoop: // get left tile number
  ...
  JSR(DrawOneSpriteRow,
      521);         // do sub to write tile numbers to first row of sprites
  ...
  a = 0x86;
  writeData(Sprite_Tilenumber + y,
            a); // otherwise remove brick tiles with lines
  writeData(Sprite_Tilenumber + 4 + y,
            a); // and replace then with lineless brick tiles
...
SetBFlip: // put block object offset back in X
  x = M(ObjectOffset);
  writeData(Sprite_Attributes + y,
            a); // store attribute byte as-is in first sprite
  a |= BOOST_BINARY(01000000);
  writeData(Sprite_Attributes + 4 + y,
            a); // set horizontal flip bit for second sprite
  a |= BOOST_BINARY(10000000);
  writeData(Sprite_Attributes + 12 + y,
            a); // set both flip bits for fourth sprite
  a &= BOOST_BINARY(10000011);
  writeData(Sprite_Attributes + 8 + y,
            a); // set vertical flip bit for third sprite
...
MoveColOffscreen:
  a = 0xf8; // move offscreen two OAMs
  writeData(Sprite_Y_Position + y,
            a); // on the left side (or two rows of enemy on either side
  writeData(Sprite_Y_Position + 8 + y,
            a); // if branched here from enemy graphics handler)
...
DChunks: // get OAM data offset
  ...
  a = M(
      Block_Rel_YPos);  // get first block object's relative vertical coordinate
  JSR(DumpTwoSpr, 525); // do sub to dump current Y coordinate into two sprites
  a = M(Block_Rel_XPos); // get first block object's relative horizontal
                         // coordinate
  ...
  c = 1;
  a -= M(ScreenLeft_X_Pos); // subtract coordinate of left side from original
                            // coordinate
  writeData(0x00,
            a); // store result as relative horizontal coordinate of original
  c = 1;
  a -= M(Block_Rel_XPos); // get difference of relative positions of original -
                          // current
  a += M(0x00);           // add original relative position to result
  a += 0x06; // plus 6 pixels to position second brick chunk correctly
  writeData(Sprite_X_Position + 4 + y,
            a); // save into X coordinate of second sprite
  a = M(Block_Rel_YPos +
        1); // get second block object's relative vertical coordinate
  writeData(Sprite_Y_Position + 8 + y, a);
  writeData(Sprite_Y_Position + 12 + y,
            a); // dump into Y coordinates of third and fourth sprites
  a = M(Block_Rel_XPos +
        1); // get second block object's relative horizontal coordinate
  writeData(Sprite_X_Position + 8 + y,
            a); // save into X coordinate of third sprite
  a = M(0x00);  // use original relative horizontal position
  c = 1;
  a -= M(Block_Rel_XPos +
         1);    // get difference of relative positions of original - current
  a += M(0x00); // add original relative position to result
  a += 0x06;    // plus 6 pixels to position fourth brick chunk correctly
  writeData(Sprite_X_Position + 12 + y,
            a);               // save into X coordinate of fourth sprite
  a = M(Block_OffscreenBits); // get offscreen bits for block object
  JSR(ChkLeftCo,
      526); // do sub to move left half of sprites offscreen if necessary
...
DrawFireball:
  ...
  writeData(Sprite_X_Position + y,
            a); // store as sprite X coordinate, then do shared code
...
DrawExplosion_Fireball:
  y = M(Alt_SprDataOffset +
        x); // get OAM data offset of alternate sort for fireball's explosion
  a = M(Fireball_State + x);   // load fireball state
...
DrawExplosion_Fireworks:
  ...
  a = 0x82;
  writeData(Sprite_Attributes + 4 + y,
            a); // set vertical flip for second sprite
  a = 0x42;
  writeData(Sprite_Attributes + 8 + y,
            a); // set horizontal flip for third sprite
  a = 0xc2;
...
DrawSmallPlatform:
  ...
  writeData(Sprite_X_Position + y, a);
  writeData(Sprite_X_Position + 12 + y,
            a); // dump as X coordinate into first and fourth sprites
  c = 0;
  a += 0x08;                               // add eight pixels
...
SwimKT: // check player's size
  ...
  a = M(Sprite_Tilenumber + 24 +
        y); // check tile number of seventh/eighth sprite
  compare(a,
          M(SwimTileRepOffset)); // against tile number in player graphics table
  if (z)
...
BigKTS: // overwrite tile number in sprite 7/8
  ...
  writeData(Sprite_Tilenumber + 24 + y,
            a); // to animate player's feet when swimming
...
FindPlayerAction:
  JSR(ProcessPlayerAction,
      533); // find proper offset to graphics table by player's actions
  goto PlayerGfxProcessing; // draw player, then process for fireball throwing

DoChangeSize:
  JSR(HandleChangeSize,
      534); // find proper offset to graphics table for grow/shrink
  goto PlayerGfxProcessing; // draw player, then process for fireball throwing
...
PlayerGfxProcessing:
  ...
  if (c)
    goto PlayerOffscreenChk; // if animation frame timer => fireball throw timer
                             // skip to end
  writeData(FireballThrowingTimer,
            a); // otherwise store animation timer into fireball throw timer
  ...
  a = M(Player_X_Speed);
  a |= M(Left_Right_Buttons); // check for horizontal speed or left/right button
                              // press
  if (z)
...
PROfsLoop: // load offscreen Y coordinate just in case
  ...
  JSR(DumpTwoSpr,
      538); // otherwise dump offscreen Y coordinate into sprite data
...
RenderPlayerSub:
  ...
  a = M(Player_Rel_XPos);
  writeData(Player_Pos_ForScroll,
            a);       // store player's relative horizontal position
...
ProcOnGroundActs:
  ...
  compare(a, 0x09);
  if (!c)
    goto ActionWalkRun;    // if less than a certain amount, branch, too slow to
                           // skid
  a = M(Player_MovingDir); // otherwise check to see if moving direction
  a &= M(PlayerFacingDir); // and facing direction are the same
  if (!z)
    goto ActionWalkRun; // if moving direction = facing direction, branch, don't
                        // skid
  ++y;                  // otherwise increment to skid offset ($03)

NonAnimatedActs:
  JSR(GetGfxOffsetAdder,
      541); // do a sub here to get offset adder for graphics table
  a = 0x00;
  writeData(PlayerAnimCtrl, a); // initialize animation frame control
  a = M(PlayerGfxTblOffsets +
        y); // load offset to graphics table using size as offset
  goto Return;
...
GorSLog: // get player's size
  ...
  a = M(ChangeSizeOffsetAdder +
        y); // get offset adder based on frame control as offset
...
GetProperObjOffset:
  ...
  c = 0;
  a += M(ObjOffsetData +
         y); // add amount of bytes to offset depending on setting in Y
...
XOfsLoop: // get pixel coordinate of edge
  ...
  a -= M(SprObject_PageLoc +
         x); // subtract from page location of object position
  x = M(DefaultXOnscreenOfs + y); // load offset value here
  compare(a, 0x00);
  if (n)
    goto XLdBData; // if beyond right edge or in front of left edge, branch
  x = M(DefaultXOnscreenOfs + 1 +
        y); // if not, load alternate offset value here
  compare(a, 0x01);
  if (!n)
...
YOfsLoop: // load coordinate for edge of vertical unit
  ...
  c = 1;
  a -= M(SprObject_Y_Position +
         x);          // subtract from vertical coordinate of object
  writeData(0x07, a); // store here
...
DrawSpriteObject:
  ...
  writeData(Sprite_Tilenumber + 4 + y,
            a); // store first tile into second sprite
...
Dump_Squ1_Regs:
  writeData(SND_SQUARE1_REG + 1,
            y); // dump the contents of X and Y into square 1's control regs
...
PlaySqu1Sfx:
  JSR(Dump_Squ1_Regs,
      565); // do sub to set ctrl regs for square 1, then set frequency regs
...
Dump_Freq_Regs:
  y = a;
  a = M(FreqRegLookupTbl + 1 +
        y); // use previous contents of A for sound reg offset
  if (z)
    goto NoTone; // if zero, then do not load
  writeData(SND_REGISTER + 2 + x,
            a);                // first byte goes into LSB of frequency divider
...
Dump_Sq2_Regs:
  writeData(SND_SQUARE2_REG,
            x); // dump the contents of X and Y into square 2's control regs
...
PlaySqu2Sfx:
  JSR(Dump_Sq2_Regs,
      566); // do sub to set ctrl regs for square 2, then set frequency regs
...
ContinueSndJump:
  a = M(
      Squ1_SfxLenCounter); // jumping sounds seem to be composed of three parts
...
ContinueSwimStomp:
  ...
  a = M(SwimStompEnvelopeData - 1 +
        y); // length of sound left, used to control sound's
  writeData(SND_SQUARE1_REG, a); // envelope
  compare(y, 0x06);
  if (!z)
    goto BranchToDecLength1;
  a = 0x9e; // when the length counts down to a certain point, put this
  writeData(SND_SQUARE1_REG + 2,
            a); // directly into the LSB of square 1's frequency divider

BranchToDecLength1:
  if (!z)
    goto DecrementSfx1Length; // unconditional branch (regardless of how we got
                              // here)
...
ContinuePowerUpGrab:
  ...
  y = a;
  a = M(PowerUpGrabFreqData - 1 +
        y); // use length left over / 2 for frequency offset
...
Square2SfxHandler:
  ...
  writeData(Square2SoundBuffer,
            y); // if found, put in buffer and check for the following
  if (n)
...
ContinueGrowItems:
  ...
  a = M(PUp_VGrow_FreqData +
        y); // use secondary counter / 2 as offset for frequency regs
...
NoStopSfx:
  x = M(AreaMusicBuffer);
  writeData(AreaMusicBuffer_Alt,
            x); // save current area music buffer to be re-obtained later
  y = 0x00;
  writeData(NoteLengthTblAdder,
            y); // default value for additional length byte offset
...
LoadHeader:
  ...
  writeData(AltRegContentFlag,
            a); // initialize alternate control reg data used by square 1
...
HandleSquare2Music:
  ...
  if (!z)
    goto MiscSqu2MusicTasks; // is it time for more data?  if not, branch to end
                             // tasks
...
EndOfMusicData:
  ...
  if (!z)
    goto NotTRO;
  a = M(
      AreaMusicBuffer_Alt); // load previously saved contents of primary buffer
  if (!z)
...
NotTRO: // check for victory music (the only secondary that loops)
  ...
  a = M(
      AreaMusicBuffer); // check primary buffer for any music except pipe intro
...
MiscSqu2MusicTasks:
  ...
  a = M(
      EventMusicBuffer); // check for death music or d4 set on secondary buffer
  a &= BOOST_BINARY(
      10010001); // note that regs for death music or d4 are loaded by default
  if (!z)
    goto HandleSquare1Music;
...
NoDecEnv1: // do a load of envelope data to replace default
  ...
  writeData(SND_SQUARE2_REG,
            a); // based on offset set by first load unless playing
...
FetchSqu1MusicData:
  ...
  a = 0x83;
  writeData(SND_SQUARE1_REG,
            a); // store some data into control regs for square 1
...
Squ1NoteHandler:
  ...
  writeData(Squ1_NoteLenCounter,
            a);              // save contents of A in square 1 note counter
...
HandleTriangleMusic:
  ...
  a = 0x1f;
  writeData(SND_TRIANGLE_REG,
            a); // load some default data for triangle control reg
...
TriNoteHandler:
  ...
  a &= BOOST_BINARY(
      01101110); // check for death music or d4 set on secondary buffer
  if (!z)
    goto NotDOrD4; // if playing any other secondary, skip primary buffer check
  a = M(
      AreaMusicBuffer); // check primary buffer for water or castle level music
  a &= BOOST_BINARY(00001010);
  if (z)
    goto HandleNoiseMusic; // if playing any other primary, or death or d4, go
                           // on to noise routine
...
NotDOrD4: // if playing water or castle music or any secondary
  ...
  if (c)
    goto LongN;
  a = M(EventMusicBuffer); // check for win castle music again if not playing a
                           // long note
  a &= EndOfCastleMusic;
  if (z)
    goto MediN;
  a = 0x0f; // load value $0f if playing the win castle music and playing a
            // short
  if (!z)
    goto LoadTriCtrlReg; // note, load value $1f if playing water or castle
                         // level music or any

MediN: // secondary besides death and d4 except win castle or win castle and
       // playing
  a = 0x1f;
  if (!z)
    goto LoadTriCtrlReg; // a short note, and load value $ff if playing a long
                         // note on water, castle
...
LoadTriCtrlReg:
  writeData(SND_TRIANGLE_REG,
            a); // save final contents of A into control reg for triangle
...
FetchNoiseBeatData:
  ...
  a = M(NoiseDataLoopbackOfs); // if data is zero, reload original noise beat
                               // offset
...
LoadEnvelopeData:
  ...
  a = M(EndOfCastleMusicEnvData +
        y); // load data from offset for win castle music
  goto Return;
...
LoadUsualEnvData:
  ...
  a = M(AreaMusicEnvData +
        y); // load default data from offset for all other music
  goto Return;

  //------------------------------------------------------------------------

LoadWaterEventMusEnvData:
  a = M(WaterEventMusEnvData +
        y); // load data from offset for water music and all other event music
  goto Return;

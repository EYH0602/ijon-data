...
void drawBox(uint32_t *buffer, int xOffset, int yOffset, int width, int height,
             uint32_t palette) {
...
void drawCHRTile(uint32_t *buffer, int xOffset, int yOffset, int tile,
                 uint32_t palette) {
...
void drawText(uint32_t *buffer, int xOffset, int yOffset,
              const std::string &text, uint32_t palette) {
...
SDL_Texture *generateScanlineTexture(SDL_Renderer *renderer) {
  ...
  SDL_Texture *scanlineTexture = SDL_CreateTexture(
      renderer, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STATIC,
      RENDER_WIDTH * 3, RENDER_HEIGHT * 3);
  uint32_t *scanlineTextureBuffer =
      new uint32_t[RENDER_WIDTH * RENDER_HEIGHT * 3 * 3];
    ...
    for (int x = 0; x < RENDER_WIDTH; x++) {
      for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
          ...
          scanlineTextureBuffer[((y * 3) + i) * (RENDER_WIDTH * 3) + (x * 3) +
                                j] = color;
        }
      }
    }
  ...
  SDL_UpdateTexture(scanlineTexture, NULL, scanlineTextureBuffer,
                    sizeof(uint32_t) * RENDER_WIDTH * 3);
...
const uint32_t *loadPalette(const std::string &fileName) {
  ...
  if (file != nullptr) {
    ...
    if (fileSize == 192 || fileSize == 1536) {
      ...
      delete[] fileBuffer;
      std::cout << "Unsupported palette file \"" << fileName << "\""
                << std::endl;
    }

    fclose(file);
  } else {
    std::cout << "Unable to open palette file \"" << fileName << "\""
              << std::endl;
  }

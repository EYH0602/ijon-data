  ...
  if (video) {
    ...
    if (!Configuration::getPaletteFileName().empty()) {
      const uint32_t *palette =
          loadPalette(Configuration::getPaletteFileName());
      if (palette) {
        paletteRGB = palette;
      }
    }
...
static void mainLoop(bool video, bool trace) {
  ...
  while (running || video) {
    ...
    // sdl_keys[SDL_SCANCODE_BACKSPACE]);
    ...
    if (video) {
      ...
      /**
       * Ensure that the framerate stays as close to the desired FPS as
       * possible. If the frame was rendered faster, then delay. If the frame
       * was slower, reset time so that the game doesn't try to "catch up",
       * going super-speed.
       */

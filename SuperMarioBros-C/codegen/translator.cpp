...
#define AUTOGENERATED_FILE_MESSAGE                                             \
...
// This sabotages the next instruction, so we need to translate these into
// goto's that skip the next instruction
//
...
Translator::Translator(const std::string &inputFilename, RootNode *astRootNode)
    : inputFilename(inputFilename), root(astRootNode) {
  returnLabelIndex = 0;
...
std::string Translator::getDataOutput() const { return dataOutput.str(); }

std::string Translator::getDataHeaderOutput() const {
  return dataHeaderOutput.str();
}

std::string Translator::getSourceOutput() const { return sourceOutput.str(); }
  ...
  for (std::list<AstNode *>::iterator it = root->children.begin();
    ...
    while (true) {
      ...
      if (child->type == AST_LABEL) {
        // Nested... classify the label as an alias, and continue trying to
        // classify the child
        //
...
void Translator::generateCode() {
  sourceOutput << "void SMBEngine::code(int mode)\n"
               << "{\n"
               << TAB << "switch (mode)\n"
               << TAB << "{\n"
               << TAB << "case 0:\n"
      ...
      if (instruction->type == AST_INSTRUCTION) {
        sourceOutput << TAB
                     << translateInstruction(
                            static_cast<InstructionNode *>(instruction));
        ...
        if (skipNextInstruction) {
          ...
          skipNextInstruction = false;
                 ...
                 strcmp(instruction->value.node->value.node->value.s, "$2c") ==
                     0) {
        // Special case: .db $2c
  ...
  for (int i = 0; i < returnLabelIndex; i++) {
    ...
    sourceOutput << TAB << "case " << indexStr << ":\n"
...
void Translator::generateConstantDeclarations() {
  constantHeaderOutput << "#ifndef SMBCONSTANTS_HPP\n"
  ...
  for (std::list<AstNode *>::iterator it = root->children.begin();
    ...
    constantHeaderOutput << "#define " << decl->value.s << " "
                         << translateExpression(decl->expression);
...
void Translator::generateDataDeclarations() {
  ...
  addressDefaults << TAB << "SMBDataPointers()\n" << TAB << "{\n";
    ...
    if (label->labelType == LABEL_DATA || label->labelType == LABEL_ALIAS) {
      if (label->labelType == LABEL_DATA) {
        ...
        while (listElement != NULL) {
          ...
          ListNode *dataListElement =
              static_cast<ListNode *>(dataItem->value.node);
          ...
          if (listElement->next != NULL) {
            if (listElement->next->value.node->type != AST_DATA16) {
              ...
              // This only occurs at the very end of the disassembly (the
              // interrupt vectors) So we will ignore it
              //
              break;
            }
          }
        ...
        // The data loading
        //
        loading << TAB << "writeData(" << labelName << ", " << labelName
        ...
        addressDefines << "#define " << labelName << " (dataPointers."
                       << labelName << "_ptr)\n";

        addressDefaults << TAB << TAB << "this->" << labelName << "_ptr = 0x"
      ...
      //
        ...
        // For aliases, we only need to worry about referencing the aliased
        // value (which will be the next label that we process, hence no
        // changing of the
  ...
  addressDefaults << TAB << TAB << "this->freeSpaceAddress = 0x" << std::hex
  ...
  //
  dataHeaderOutput << "#ifndef SMBDATAPOINTERS_HPP\n"
...
std::string Translator::translateBranch(const std::string &condition,
...
std::string Translator::translateExpression(AstNode *expr) {
  ...
  if (expr != NULL) {
    ...
    case AST_ADD: {
      ...
      result = translateExpression(node->lhs) + " + " +
               translateExpression(node->rhs);
    } break;
    case AST_SUBTRACT: {
      BinaryNode *node = static_cast<BinaryNode *>(expr);
      result = translateExpression(node->lhs) + " - " +
    ...
    if (strcmp(inst->value.s, "JumpEngine") == 0) {
      ...
      while (listElement != NULL) {
        ...
        if (listElement->value.node->lineNumber != 0) {
          const char *comment =
              lookupComment(listElement->value.node->lineNumber);
          if (comment) {
            // Strip the first ';' character
            //
            result += " // ";
...
std::string Translator::translateOperand(AstNode *operand) {
  ...
  if (operand != NULL) {
    // If immediate addressing is not the underlying node, it means read from
    // memory
    //

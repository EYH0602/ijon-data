...
void mapNewline(int line) { newlineSet.insert(line); }
...
const char *lookupComment(int line) {
  ...
  if (it != commentMap.end()) {
    // We need to duplicate the string since erase() removes the original copy
    // of it
    //
    comment =
        duplicateString((*it).second.c_str(), strlen((*it).second.c_str()));

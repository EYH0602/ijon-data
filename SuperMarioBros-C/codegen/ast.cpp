...
UnaryNode::UnaryNode(AstType type, AstNode *child) : AstNode(type) {
  this->child = child;
}
...
RootNode::RootNode() : AstNode(AST_ROOT) {}

ListNode::ListNode() : AstNode(AST_LIST) { this->next = NULL; }

DeclNode::DeclNode(const char *name, AstNode *expression) : AstNode(AST_DECL) {
  this->value.s = name;
  this->expression = expression;
}

LabelNode::LabelNode(const char *name, AstNode *child) : AstNode(AST_LABEL) {
  this->labelType = LABEL_NONE;
  this->value.s = name;
  this->child = child;
}

InstructionNode::InstructionNode(int code) : AstNode(AST_INSTRUCTION) {
  this->code = code;
}
